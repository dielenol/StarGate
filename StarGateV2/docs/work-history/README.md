...
`pnpm work-history:audit`은 작업 이력 규칙 도입 커밋 `78dd3969` 이후의 사용자 화면 코드 커밋과 `관련 커밋` 기록을 비교한다. 범위를 좁히려면 `--base <ref> --head <ref>`를 전달한다.

감사기는 문서화된 merge에 포함된 구현, 이미 기록된 커밋과 동일한 patch, 최종 상태에서 되돌린 변경, 아직 화면 소비처가 연결되지 않은 정적 자산과 페이지 범위 밖 VTT API를 예외 처리한다. 휴리스틱 검사이므로 누락 후보가 있어도 종료 코드는 0이며, GitHub Actions에서도 경고만 남긴다. 감사기 실행 자체가 실패한 경우에만 CI가 실패한다.

감사기 회귀 테스트는 `pnpm work-history:test`로 실행한다.
...
- [성능](shared/performance.md)
- [성능 관측](shared/performance-observability.md)
- [ERP realtime](shared/realtime.md)
