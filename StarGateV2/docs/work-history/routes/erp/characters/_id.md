...
## 2026-09-08 · 갱신 개선 · 편집 후 대시보드 정보

- 캐릭터 편집·삭제와 play/lore 저장 성공 시 대시보드 캐시도 무효화해 재진입 시 최신 프로필·상태를 조회한다. 기존 목록·상세·변경 이력 캐시 갱신은 유지한다.
- 검증: 성공 경로 독립 리뷰, `pnpm typecheck`, `pnpm lint`. 대시보드 재진입 갱신은 Dossier 모의 저장 브라우저 흐름으로 확인했으며 실제 캐릭터 편집·삭제는 실행하지 않았다.
- 관련 구현 커밋: `032503b7`
