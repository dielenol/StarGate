...
## 2026-08-27 · 밸런스 조정 · NOVEX 고변동성 시장

- 평시 랜덤 변동을 `±5%`, 기본 변동 상한을 `±7%`, 일반 회차 상한을 `±10%`로 높이고 평균회귀를 `10%`로 완화했다. 수급은 `4% × tanh(순매수/150)`로 강화했다.
- 자동 일반 공시는 `4~9%`, 충격 공시는 `10~25%`로 확대하고 공시·충격 회차 상한과 자동 냉각 기준을 각각 `15%`·`25%`·`15%`로 조정했다. 공시 분포는 일평균 `2.04건`, 발생일 `92%`, 하루 최대 4건·충격 1건으로 유지했다.
- 자동 특별공시는 랜덤·수급에 상쇄되지 않고 지정 등락률을 그대로 적용하며, 가려진 평시 변동과 수급은 다음 일반 회차로 이월한다. 회차 기여 장부 합계는 실제 등락과 일치하도록 보정했다.
- 검증: core 41건, shared NOVEX 26건, worker 156건, 20만 일 시뮬레이션, `pnpm typecheck`, 전체 `pnpm lint`, production build 108페이지, critical risk review 재검토 통과
- 관련 구현 커밋: `8c22c17f`
- 운영 경계: 라이브 시세·공시 큐·수급·DB·outbox·Discord·배포는 변경하지 않았다. replica-set 전용 통합 테스트 2건은 환경 부재로 skip했다.
