...
## 2026-08-13 · 안전성 강화 · 정지 종목 자산 교환 차단

- 거래정지 또는 시세 미등록 종목은 즉시 전달, 신규·수정 제안 저장, 양측 첫 확정과 최종 체결에서 서버가 차단하며 모든 자산 mutation 전에 종목 상태를 확인한다.
- 진행 중 교환의 양측 제안 종목 상태를 ETag 응답에 포함하고 거래 불가 배지·확정 잠금·제안자별 제거 안내를 제공해 정지 후에도 화면이 최신 상태로 수렴하도록 했다.
- 각 참여자는 상대 제안의 정지 상태와 무관하게 자기 제안에서 문제 종목을 제거할 수 있고, 양측 제안이 모두 유효해진 뒤에만 다시 확정할 수 있다.
...
## 2026-09-08 · 기능 보강 · 처리할 교환과 상세 진입

- 홈에는 현재 MAIN의 OPEN 교환 중 내가 최신 조건을 확정하지 않은 건만 표시한다. 상대 확정 여부를 안내하며 이미 내가 확정한 상대 대기는 처리할 일에서 제외한다.
- `tradeId` 링크로 정확한 진행 중 거래 또는 이력에 초점을 맞춘다. 최근 100건 밖의 거래도 참여자 본인에게만 추가 조회하며 다른 사람의 거래와 없는 ID는 같은 결과로 처리한다.
- 교환 mutation 성공 시 홈 요약을 갱신하고 실패는 성공으로 처리하지 않는다.
- 검증: 권한·조회·실제 QueryClient mutation/invalidation 테스트를 포함한 집중 테스트 43/43, 타입 검사·린트·프로덕션 빌드. 인증 브라우저에서 상세 링크·잘못된 ID·뒤로/앞으로·모바일을 확인했고, 로컬 mock 확정 성공 후 60초 이전 홈 갱신과 mock 409 실패 후 업무 유지를 검증했다.
- 관련 구현 커밋: `d7c28636`
- 운영 경계: 브라우저 mutation은 로컬 mock으로만 응답했으며 라이브 교환·크레딧·인벤토리는 변경하지 않았다.
