...
## 2026-08-17 · UI 개선 · 외부 조직 drilldown 팔레트

- 적대세력 화면의 위험색을 `ClearanceStrip`까지 확장하고, 등급 pill과 `CLASSIFIED`의 의미색은 기존 계약을 유지했다.
- 같은 날 앞선 작업에서 기본 골드로 유지했던 군부·시민사회 하위 화면을 각각 스틸 블루·화이트 팔레트로 전환해 breadcrumb, 그룹 헤더, 하위 조직 hover·focus·open 상태와 인물 카드까지 통일했다.
- 검증: 조직 tone·CSS 전파 회귀 테스트 5건, `pnpm typecheck`, `pnpm lint`, 인증된 `localhost:43849`의 아넨에르베·USA·백장미단·스페이스제로 하위 화면과 hover·키보드 focus·open 상태, 1280×900·700×900 가로 넘침 0, 브라우저 콘솔 오류·경고 0
- 관련 커밋: `e7f9590a`

## 2026-08-30 · UI 개선 · 로어 기반 기관·세력 문장

- 사무국·현장요원, 통제·재무 기구, 알파~에코 섹터, 세계이사회·군부·시민사회·적대세력과 NOGA·미국·러시아·황금여명회·아넨에르베의 18개 문양을 교체했다.
- 사무국의 행정·통제, 섹터별 임무, 황금여명회의 가면·무대막, 아넨에르베의 표본·기록 등 기존 설정을 디자인 근거로 삼고 미확정 교리나 공식 문장을 새 canon으로 등록하지 않았다.
- OrgIcon inline source·공개 SVG mirror·NOVUS 마스터를 같은 body로 유지했다. 기존 조직 code·계층·분류·색상은 변경하지 않았다.
- 검증: 동기화 계약 36건·기존 외부 조직 테스트 3건, 아이콘 감사 128종·38경로, XML·master JS, `pnpm typecheck`, `pnpm lint`, 인증된 1440×1000·390×844 조직도와 사무국·MANUS·군부·적대세력 하위 화면의 실제 body·색상 상속·가로 넘침·브라우저 오류 확인
- 관련 구현 커밋: `eb144b4e`
- 운영 경계: faction·institution·NPC·라이브 DB 레코드는 변경하지 않았다.
