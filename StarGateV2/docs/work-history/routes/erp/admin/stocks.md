...
## 2026-08-19 · 기능 추가 · 종목별 시나리오 흐름

- 공시와 기업행동이 서로 다른 컬렉션에 저장돼 예약된 전개를 한눈에 볼 수 없던 문제를 해소했다. 종목을 고르면 예약된 공시·배당·분할·유상증자를 회차 시각 순으로 쌓아 보여주고 각 단계의 예상 가격을 함께 표시한다.
- 같은 회차에서는 주식 수가 변하는 구조 변경을 먼저 반영한 뒤 공시를 적용하고, 한 공시에 종목 효과와 시장 전체 효과가 함께 있으면 종목 효과를 우선한다.
- 배당은 배당락, 분할은 비율 나눗셈, 유상증자는 조정률로 반영하며 최소 단가 아래로는 내려가지 않는다. 계산은 미리보기 전용이고 회차 확정은 worker 엔진이 단독 소유한다.
- 카드에서 바로 예약을 취소할 수 있고 멱등 키를 재사용해 중복 요청을 막는다.
- 검증: 시나리오 흐름 결정 테스트 9건 신규, 웹 주식·거래 계약 70건, `pnpm typecheck`, 전체 `pnpm lint`, production build
- 관련 커밋: `f376b8f5`
- 운영 경계: 라이브 공시·기업행동 취소는 실행하지 않았다.
