...
## 2026-08-30 · 성능·UI 개선 · 선택 조회와 공적 타임라인

- 개요를 제외한 각 탭은 해당 부문의 RSC 초기 데이터와 Query만 조회한다. 비활성 탭의 polling은 멈추되 root invalidation과 재진입 시 갱신, 마지막 성공 데이터 보존은 유지한다. 개요와 연구 패널의 중복 스냅샷 조회는 요청 수명 안에서만 합친다.
- 공적 원본 확인을 보고서·공개 인물·후보·소유자 일괄 조회로 바꾸고 해시에 필요 없는 위키·카탈로그 참조는 읽지 않는다. 현재 U 등급, 코드네임 중복, AGENT·ACTIVE owner, 원본 해시 검증과 공개 응답 필드는 보존한다.
- 실제 활성 공적 13건·고유 보고서 10건으로 20회 교차 측정해 원본 material과 공개 항목 전체가 동일함을 확인했다. 원장 목록 조회부터 원본 확인까지 p50은 490.13→237.27ms, p95는 604.85→251.29ms였다. 전체 페이지 로딩 시간이나 Web Vitals 수치는 아니다.
- 반복된 공적 카드를 번호·NOVUS 인장·점선 축을 가진 연속 타임라인으로 정리하고 전체 보드의 backdrop blur를 제거했다. 화면 밖 목록 렌더를 지연하고 비활성 캐러셀 슬라이드는 `inert`로 키보드 탐색에서 제외한다.
- 390px에서 부문 헤더가 개요의 2열 규칙을 상속해 제목이 세로로 찌그러지는 문제와 인장에 번호 스타일이 함께 적용되는 문제를 수정했다.
- 검증: shared-db·Hall·대시보드·Query·realtime 집중 62건 통과/전용 DB 통합 1건 환경 미설정 skip, core 44건·worker 132건, `pnpm typecheck`, `pnpm lint`, `pnpm build`, `pnpm --dir .. build:worker`, critical risk review P0–P2 없음
- 브라우저: 인증 GM의 2560×1440·1280×720·1024×768·390×844, 1280px 최대 보드 폭, 모바일 연구 보드 859px·순서 1→2→3·초상 3개 정상, 필터 0/4/13건 및 back/forward, 내 리본 빈 상태, 잘못된 cursor 오류·정상 주소 복구, 원본 보고서 이동·공적 인용 패널과 정상 경로 콘솔 오류 0건 확인
- 관련 구현 커밋: `45d9fc0a`
- 운영 경계: 운영 DB·인덱스·공적 내용·알림·웹훅은 변경하지 않았다. 푸시·배포는 이 작업에 포함하지 않는다.
