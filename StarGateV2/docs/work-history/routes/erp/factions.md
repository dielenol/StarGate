...
## 2026-08-30 · UI 개선 · 외부 조직 헤럴딕 문장 동기화

- 미국·러시아·NOGA·황금여명회·아넨에르베 노드가 신원조회와 동일한 로어 기반 문양을 사용하도록 교체하고, 적대세력 공용 fallback도 같은 문장으로 맞췄다.
- 조직별 기존 로고 이미지, 노드 분류, 우호도와 접선·연락 동작은 변경하지 않았다.
- 검증: 동기화·외부 조직 계약 39건, XML·아이콘 감사·master JS, `pnpm typecheck`, `pnpm lint`, 인증된 1440×1000 세력도에서 5개 조직의 실제 SVG body·색상 상속과 가로 넘침·깨진 이미지·브라우저 오류 0건 확인
- 관련 구현 커밋: `eb144b4e`
- 운영 경계: 우호도 저장·접선·라이브 데이터 mutation은 실행하지 않았다.
