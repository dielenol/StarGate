<!-- 소모품 템플릿 — /create-lore consumable 로 자동 생성 가능. 필드 명세: packages/shared-db/src/schemas/consumable.schema.ts 참조 -->
<!--
  frontmatter 필드 안내
  ─────────────────────
  필수: code, slug, name, category, price, description, isAvailable, isPublic
  권장: nameEn, effect, tags, source, previewImage, createdAt, updatedAt
...
-->
---
code: EXAMPLE_CONSUMABLE
slug: example-consumable
name: 예시 소모품
nameEn: Example Consumable
category: CONSUMABLE
price: 0
effect:
...
## 배경
이 소모품의 유래·서사 (옵션). master_items.lore.background 로 적재.

## 획득 경로
어떻게 얻을 수 있는지 (옵션). 편의점/임무/이벤트 보상 등. master_items.lore.acquisition.
