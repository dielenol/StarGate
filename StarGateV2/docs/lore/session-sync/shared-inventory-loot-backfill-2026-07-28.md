...
- 화이트 로즈 조력자 호출권은 수량 1의 권한 보유 항목으로만 등록하며, 사용 후 소모 여부는 확정하지 않는다.
...
- 신규 카탈로그 7건은 모두 `isAvailable: false`로 유지한다. 일반 지급 API에는 노출하지 않고 공용 인벤토리의 보관 전리품으로만 표시한다.

## Visual Asset Ledger

| 대상 | report | report wiki mirror | dedicated wiki | catalog | Dossier/personnel |
|---|---|---|---|---|---|
| 신규 카탈로그 7건 | 미변경 | 미변경 | 미변경 | 정확한 카탈로그 자산 미제공으로 `previewImage` 비움 | 해당 없음 |
