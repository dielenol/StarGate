...
- 구현 전 대상 route의 이력 문서, 가장 가까운 상위 route 문서와 관련 공용 이력 문서를 확인한다.
- 최근 30일 안에 같은 목적의 완료 이력이 있으면 연결된 커밋 이후 관련 코드가 바뀌었는지 먼저 비교한다.
- 관련 코드가 실질적으로 바뀌지 않았다면 같은 구현을 반복하지 않는다. 기존 작업일·변경 내용·검증·커밋을 사용자에게 알리고 재검증 또는 재실행이 필요한지 확인한다.
- 사용자가 회귀를 제보했거나 명시적으로 재검증을 요청한 경우에는 이력만 믿지 말고 현재 동작을 다시 재현한다.
- 기능 개발, 버그 수정, 성능 최적화, 접근성·반응형 개선처럼 동작이 달라진 완료 작업만 기록한다. 조사만 한 작업, 임시 디버깅, 포매팅만 한 변경은 기록하지 않는다.
- 검증된 구현 커밋을 먼저 만든 뒤 페이지 문서 끝에 날짜순으로 이력을 추가하고, 관련 구현 커밋 해시를 적은 문서 커밋을 별도로 만든다.
- 여러 페이지에 영향을 주면 각 페이지에 사용자 관점의 결과를 기록한다. 페이지로 귀속할 수 없는 공용 변경만 `docs/work-history/shared/`에 기록한다.

## Authenticated Browser QA

- 로컬 ERP 인증 브라우저 테스트는 `http://localhost:43849`를 사용한다. `AUTH_URL`과 쿠키 호스트가 달라지는 `127.0.0.1`은 사용하지 않는다.
