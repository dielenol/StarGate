...
function LoginForm() {
  ...
  const [loadingProvider, setLoadingProvider] = useState<
    "credentials" | "discord" | "guest" | null
  >(null);
  ...
  return (
    <div className={styles.login}>
      ...
      <form className={styles.login__form} onSubmit={handleCredentialsLogin}>
        ...
        >
          {loadingProvider === "credentials" ? "인증 처리 중..." : "로그인"}
        </button>
      </form>
      ...
      >
        ...
        {loadingProvider === "discord" ? "연결 중..." : "Discord로 로그인"}
      </button>

      <button
        className={styles.login__guest}
        type="button"
        onClick={handleGuestLogin}
        disabled={loadingProvider !== null}
      >
        {loadingProvider === "guest" ? "미리보기 준비 중..." : "게스트로 둘러보기"}
      </button>
      <p className={styles["login__guest-note"]}>
        계정 없이 공개 범위의 ERP를 조회할 수 있습니다. 데이터 변경은 제한됩니다.
      </p>
