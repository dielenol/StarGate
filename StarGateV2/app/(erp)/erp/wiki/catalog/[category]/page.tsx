...
type Category = "equipment" | "consumable";

type ItemCategoryFilter = "WEAPON" | "ARMOR" | "CONSUMABLE";

const CATEGORY_MAP: Record<Category, ItemCategoryFilter[]> = {
...
function isCategory(value: string): value is Category {
  return value === "equipment" || value === "consumable";
}
  ...
  return (
    <CatalogClient
      ...
      initialItems={items
        ...
        .filter(
          (it): it is typeof it & { category: ItemCategoryFilter } =>
            itemCategories.includes(it.category as ItemCategoryFilter),
        )
        .map((it) => ({
          _id: it._id?.toString() ?? "",
          slug: it.slug,
