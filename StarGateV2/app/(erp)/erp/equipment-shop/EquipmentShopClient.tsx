    ...
    (event: TowaskiQualificationDialogueEvent) => {
      ...
      if (event.type === "start") {
        const instruction = {
          ...
          precision:
            "축소된 표적을 제한 시간 안에 식별하고 한 발씩 명중시키십시오.",
          heavy:
            "심하게 흔들리는 가상 조준점을 보정해 적성 표적만 사격하십시오.",
          flame:
            "3칸 소이선을 배치해 적성 경로를 차단하고 안전 구역을 피하십시오.",
          sonic:
            "판정선에 도달한 공진 노트에 맞춰 펄스를 입력하고 보호 박자는 넘기십시오.",
          explosive:
            "수류탄·로켓 점검값을 확인해 반출·정비·격리로 분류하십시오.",
        }[event.mode ?? "firearm"];
        setNotice({
          tone: "info",
          title: "자격시험 시작",
