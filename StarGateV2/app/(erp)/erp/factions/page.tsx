...
import type { Character, FactionCode } from "@/types/character";
import { FACTIONS } from "@/types/character";
...
import { getFactionScope, getTopLevelGroup, isFaction } from "@/lib/org-structure";
...
type ExternalFactionCode = (typeof EXTERNAL_FACTION_CODES)[number];
...
function isExternalFactionCode(code: string): code is ExternalFactionCode {
  return EXTERNAL_FACTION_CODES.includes(code as ExternalFactionCode);
}
...
function resolveExternalGroup(c: Character): ExternalFactionCode | null {
  const externalSubOrg = resolveExternalSubOrg(c);
  if (externalSubOrg && isExternalFactionCode(externalSubOrg.parentCode)) {
    return externalSubOrg.parentCode;
  }

  const dept = c.department;
  if (dept && dept !== "UNASSIGNED") {
    const top = getTopLevelGroup(dept);
    if (isExternalFactionCode(top)) return top;
  }

  if (c.factionCode && isFaction(c.factionCode)) {
    const scope = getFactionScope(c.factionCode);
    if (scope === "external" && isExternalFactionCode(c.factionCode)) {
...
function buildFactionNodes(
  ...
  return EXTERNAL_FACTION_CODES.map((code) => {
    const faction = FACTIONS.find((f) => f.code === code);
    return {
      code,
      ...
      memberCount: groupCounts[code] ?? 0,
      contactCount: contactBuckets[code]?.length ?? 0,
      wikiCount: wikiBuckets[code]?.length ?? 0,
      signalCount: signalBuckets[code]?.length ?? 0,
    };
  });
}

function buildSubOrgNodes(
  groupCounts: Record<string, number>,
  contactBuckets: Record<string, FactionBoardContact[]>,
  wikiBuckets: Record<string, FactionBoardLink[]>,
  signalBuckets: Record<string, FactionBoardSignal[]>,
): FactionBoardNode[] {
  return EXTERNAL_SUB_ORGS.map((org) => ({
    code: org.code,
    label: org.label,
    labelEn: org.labelEn,
    ...
    logoUrl: org.logoUrl,
    memberCount: groupCounts[org.code] ?? 0,
    contactCount: contactBuckets[org.code]?.length ?? 0,
    wikiCount: wikiBuckets[org.code]?.length ?? 0,
    signalCount: signalBuckets[org.code]?.length ?? 0,
  }));
}
...
async function FactionsBody({ role }: { role: UserRole }) {
  ...
  for (const raw of visibleCharacters) {
    const groupCode = resolveExternalGroup(raw);
    if (!groupCode) continue;

    const subOrg = resolveExternalSubOrg(raw);
    const subOrgCode = subOrg?.code ?? null;
    ...
    const contact: FactionBoardContact = {
      ...
      groupCode,
      subOrgCode,
      subOrgLabel: subOrg?.label ?? null,
      profileHref: id
        ? `/erp/personnel/${id}`
        : `/erp/personnel?group=${groupCode}`,
    };

    groupCounts[groupCode] = (groupCounts[groupCode] ?? 0) + 1;
    pushContact(contactBuckets, groupCode, contact);

    if (subOrgCode) {
      groupCounts[subOrgCode] = (groupCounts[subOrgCode] ?? 0) + 1;
      pushContact(contactBuckets, subOrgCode, contact);
    }
  }
  ...
  for (const page of rawWikiPages) {
    ...
    for (const code of [...EXTERNAL_FACTION_CODES, ...EXTERNAL_SUB_ORGS.map((org) => org.code)]) {
  ...
  const factionNodes = buildFactionNodes(
    groupCounts,
    contactBuckets,
    wikiBuckets,
    signalBuckets,
  );
  const subOrgNodes = buildSubOrgNodes(
    groupCounts,
    contactBuckets,
    wikiBuckets,
    signalBuckets,
  ...
  const totals: FactionBoardTotals = {
    factionCount: factionNodes.length,
    subOrgCount: subOrgNodes.length,
    memberCount: EXTERNAL_FACTION_CODES.reduce(
      (sum, code) => sum + (groupCounts[code] ?? 0),
      0,
    ),
    contactCount: EXTERNAL_FACTION_CODES.reduce(
      (sum, code) => sum + (contactBuckets[code]?.length ?? 0),
      0,
    ),
