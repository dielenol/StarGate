...
const COUNCIL_DIALOGUE = buildFactionDialogue({
  ...
  idleLines: [
    ...
    "보고서는 읽어 봤죠. 다만 명분과 책임 주체가 아직 흐리군요.",
    "안건은 봤습니다. 이사회가 보호할 이유를 한 줄로 정리해 주시죠.",
    "최근 기록 덕분에 안건을 올릴 명분은 생겼군요. 필요한 보호 범위를 말씀하시죠.",
    "당신 쪽 안건은 우선 검토할 수 있습니다. 후원 비용과 공개 책임부터 맞춰야겠군요.",
...
const MILITARY_DIALOGUE = buildFactionDialogue({
  ...
  idleLines: [
    "지원은 없어. 현재 편성과 철수 계획만 보고해.",
    "요청 확인. 보급보다 먼저 지휘선과 퇴로를 증명해.",
    "지원 요청 확인. 돌아올 길부터 말해, 철수 계획 없는 작전엔 지원 없어.",
...
const CIVIL_DIALOGUE = buildFactionDialogue({
  ...
  idleLines: [
    ...
    "사람들이 아직 경계하는 중이에요. 누가 다쳤고 무엇이 남았는지부터 들려주세요.",
    "현장 사람들부터 얘기해 주세요. 피해가 어디에 남았는지 알아야 연결할 곳을 찾을 수 있어요.",
    "도울 수 있는 지역 접점을 찾아볼게요. 당사자 동의도 함께 확인하죠.",
    "몇 곳에서 당신 쪽 기록을 믿기 시작했네요. 이번엔 피해자에게 필요한 것을 먼저 묻죠.",
...
const SPACE_ZERO_DIALOGUE = buildFactionDialogue({
  ...
  idleLines: [
    "현재 조건으로는 책임 범위가 맞지 않네요. 자산보다 회수 계획부터 다시 가져오시죠.",
    "기술 제안은 흥미롭군요. 다만 손실 책임과 회수 조건이 비어 있군요.",
    "조건은 이해했습니다. 회수와 책임 범위만 맞으면 계약은 빨라집니다.",
    "현장 검증 조건은 조율할 수 있겠군요. 운용 기록을 어디까지 공유할지 정해 주세요.",
    "당신 쪽 요청은 우선 검토 대상입니다. 납기보다 회수 기준을 먼저 확정하죠.",
    "전략 계약으로 검토하겠습니다. 권리와 책임이 같은 문장에 들어가야 체결되죠.",
  ],
...
const GOLDEN_DAWN_DIALOGUE = buildFactionDialogue({
  ...
  errorLine: "분석 중단. 원신호는 보존했지만 단서로 확정하진 않았습니다.",
});
...
const AHNENERBE_DIALOGUE = buildFactionDialogue({
  ...
  errorLine: "대조가 중단됐군요. 자료는 보존했지만 연결성은 확정하지 않았습니다.",
});
