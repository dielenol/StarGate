...
function Lightbox({ item, album, index, total, onClose, onPrevious, onNext }: { item: GalleryItemDto; album: GalleryAlbumDto | null; index: number; total: number; onClose: () => void; onPrevious: () => void; onNext: () => void }) {
  ...
  return <div className={styles.overlay} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className={styles.lightbox} ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId}><button className={styles.modal__close} onClick={onClose} aria-label="상세 보기 닫기">×</button><div className={styles.lightbox__stage}>{total > 1 && <button className={`${styles.lightbox__edge} ${styles["lightbox__edge--previous"]}`} onClick={onPrevious} aria-label="이전 이미지">‹</button>}<div className={styles.lightbox__image}><Image src={item.image.fullSrc} alt={item.image.alt} width={item.image.width ?? 1600} height={item.image.height ?? 1200} sizes="(max-width: 680px) 100vw, 72vw" unoptimized={item.kind === "FANART"} priority /></div>{total > 1 && <button className={`${styles.lightbox__edge} ${styles["lightbox__edge--next"]}`} onClick={onNext} aria-label="다음 이미지">›</button>}<span className={styles.lightbox__counter}>{String(index + 1).padStart(2, "0")} / {String(total).padStart(2, "0")}</span></div><aside className={styles.lightbox__meta}><div className={styles.lightbox__eyebrow}><span>{item.kind === "SESSION" ? "SESSION ARCHIVE" : "FAN ART ARCHIVE"}</span>{album && <Link href={album.href}>{album.reportNumber}</Link>}</div><h2 id={titleId}>{item.title}</h2><p className={styles.lightbox__description}>{item.description || "기록된 설명이 없습니다."}</p>{item.kind === "FANART" && <p className={styles.lightbox__artist}>ARTIST · {item.artistName}</p>}{album && <p className={styles.lightbox__album}>{album.title}</p>}<div className={styles.lightbox__tags}>{item.tags.slice(0, 6).map((tag) => <Tag key={tag}>{tag}</Tag>)}{item.tags.length > 6 && <Tag>+{item.tags.length - 6}</Tag>}</div>{total > 1 && <div className={styles.lightbox__navigation}><button onClick={onPrevious}>← 이전</button><button onClick={onNext}>다음 →</button></div>}</aside></div></div>;
}
