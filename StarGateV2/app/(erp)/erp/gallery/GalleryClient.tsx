...
export default function GalleryClient({ initialData, initialDataUpdatedAt }: Props) {
  ...
  return (
    <section className={styles.gallery} aria-label="갤러리">
      ...
      <div className={styles.grid} aria-busy={query !== deferredQuery}>{items.map((item, index) => <GalleryCard key={item.id} item={item} album={item.albumSessionId ? albumBySessionId.get(item.albumSessionId) ?? null : null} deleteError={deleteError?.id === item.id ? deleteError.message : ""} eager={index === 0} isDeleting={deletingId === item.id} onOpen={() => setSelectedId(item.id)} onEdit={() => { setEditing(item as GalleryFanartItemDto); setEditorMode("edit"); }} onModerate={() => { setEditing(item as GalleryFanartItemDto); setEditorMode("moderate"); }} onDelete={() => { if (item.kind === "FANART") void deleteFanart(item); }} />)}</div>
...
function Lightbox({ item, album, index, total, onClose, onPrevious, onNext }: { item: GalleryItemDto; album: GalleryAlbumDto | null; index: number; total: number; onClose: () => void; onPrevious: () => void; onNext: () => void }) {
  ...
  return <div className={styles.overlay} role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className={styles.lightbox} ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId}><button className={styles.modal__close} onClick={onClose} aria-label="닫기">×</button><div className={styles.lightbox__image}><Image src={item.image.fullSrc} alt={item.image.alt} width={item.image.width ?? 1600} height={item.image.height ?? 1200} sizes="100vw" unoptimized={item.kind === "FANART"} priority /></div><div className={styles.lightbox__meta}><div><span>{index + 1} / {total} · {item.kind === "SESSION" ? "SESSION" : "FAN ART"}</span><h2 id={titleId}>{item.title}</h2><p>{item.description || "설명 없음"}</p>{item.kind === "FANART" && <p>ARTIST · {item.artistName}</p>}{album && <Link href={album.href}>{album.reportNumber} · {album.title}</Link>}</div><div className={styles.lightbox__navigation}><Button size="sm" onClick={onPrevious} aria-label="이전 이미지">← 이전</Button><Button size="sm" onClick={onNext} aria-label="다음 이미지">다음 →</Button></div></div></div></div>;
}
