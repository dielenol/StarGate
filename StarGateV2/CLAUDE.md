...
사용자에게 보이는 페이지 동작을 실질적으로 변경할 때는 [페이지별 작업 이력 규칙](docs/work-history/README.md)을 따른다.

- 구현 전 대상 route, 가장 가까운 상위 route와 관련 공용 문서의 최근 30일 이력을 확인한다.
- 같은 목적의 완료 이력 이후 관련 코드가 바뀌지 않았다면 구현을 반복하지 말고 기존 이력과 커밋을 먼저 알린다.
- 검증된 구현 커밋 뒤 해당 페이지 문서에 새 이력을 추가하고, 구현 커밋 해시를 연결한 문서 커밋을 별도로 만든다.
