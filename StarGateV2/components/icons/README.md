...
## novus-icons v1 동기화 (2026-06-13)

마스터 `novus-icons v1.html` (ID 0–41, stroke 1.5 / 24×24 / currentColor) 기준 대조 결과.

### 적용 상태

- **barrel(ic_*.svg) 24종 — 변경 없음 (이미 마스터와 일치)**: core-archive/containment/sample · faction-map/hostile/faction-briefing/relations/affinity · inventory-equipment/-consumable/-misc · transactions · grid-all/basic/recovery/preference/rare · concept/wiki-faction/institution/place/regulation/goods · person-card
- **OrgIcon 인라인 세트 — `HOSTILE` 1종 교체**: 구형(다이아몬드+X) → 마스터 ID 23(교차 검). barrel `IconHostile`(ic_hostile.svg)과 도면 통일 — 세력도 페이지에서 두 도형이 혼재하던 문제 해소. 미러 `org_faction_hostile.svg` 신규 생성(OrgIcon 미러 규칙 충족). 나머지 16종(ROOT~MINI)은 이미 일치.

### 보완 필요 — 마스터에 있으나 미적용 (적용처 결정 대기)

| 마스터 ID | key | 표기 사유 / 적용 후보 |
|---|---|---|
| 0 | `IconEmblem` | 미적용. 본부 노드는 OrgIcon `NOVUS_ORDO`(지구본 모티프, 마스터 미수록)를 사용 중 — 교체/병행 결정 필요 |
| 2 | `IconZulu` | 미적용. 위키 `개체`/`줄루` 카테고리가 현재 `IconContainment`(ID 3)를 대용 중 (`WikiClient.tsx` WIKI_CATEGORY_ICONS) — **도입 후보 1순위** |
| 4 | `IconClearance` | 미적용. 신원조회 보안등급 UI(ClearanceStrip/ClearanceMap) 후보 |

### 이원화 잔존 — 마스터 범위 밖 (삭제하지 않음, 통일 여부만 결정 필요)

- `ic_equipment.svg`(방패+체크): 기록보관소 카탈로그 WEAPON/ARMOR + 장비판매점 nav 전용. 마스터 equipment(ID 27 = `ic_inventory-equipment.svg`, 위키 장비 카테고리에 적용됨)와 도면 이원화. 마스터에 "기록보관소 카탈로그" 그룹이 없어 그대로 유지.
- `ic_consumable.svg`(플라스크): 카탈로그 CONSUMABLE 전용. 마스터 consumable(ID 28)과 이원화 — 위와 동일 사유로 유지.
- `ic_archive.svg`(파일 캐비닛): 카탈로그 전체 탭 / 갤러리 nav / 공개 사이드바. 마스터 archive(ID 1)는 `ic_core-archive.svg`에 적용돼 있음 — 구형 도면 잔존.
- OrgIcon `NOVUS_ORDO` / `MANUS` / `SECTOR_A~E`: 사이트 전용 아이콘 (마스터 미수록) — 유지.
