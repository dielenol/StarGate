...
const TEMPER_BLOCKED_LINES: Record<TemperBlockReason, readonly string[]> = {
  ...
  qualification: [
    ...
    "이건 아직 못 내줘. 장비가 손목부터 끌고 가겠네—훈련장에서 멈추는 순서부터 맞추고 와.",
  ],
  insufficient: [
  ...
  checkoutError: [
    ...
    "여기서 한 번 끊자. 같은 절차를 밀어붙여 봐야 장비만 상하니까, 장부부터 다시 맞춰 와.",
  ],
};
...
> = {
  assault: [
    "공격 쪽을 살릴 거면 앞으로 끌려가지 않는 규격부터 보자.",
    "힘을 싣고 싶어? 그럼 휘두른 뒤에도 멈출 수 있는 무기를 골라야지. 시작보다 끝이 더 비싸거든.",
    "빠른 첫 동작이 필요하면 회수할 때 자세 안 무너지는 길이부터 맞춰 보자.",
  ],
  guard: [
    "받아낼지 흘릴지부터 정하자. 둘 다 원하면 값도 무게도 올라가.",
    "방호 자세에서 손이 묶이지 않는 규격으로 볼까? 중심은 내가 맞춰 줄게.",
    "충격을 정면으로 받을 거라면 관절까지 밀리지 않는 손잡이로 가자. 손목은 재고가 없거든.",
  ],
  endurance: [
    "오래 쓸 장비라면 첫 타격보다 열 번째에도 중심 남는 걸로 보자. 그게 진짜 가성비야.",
    "장기 임무용이야? 손잡이 마찰이랑 관절 피로까지 맞춰 줄게.",
    "긴 임무에선 작은 불편이 사람부터 갉아먹어. 오래 쥐어도 덜 억울한 규격으로 보자.",
  ],
  focus: [
    "정밀한 날을 찾는다면 회수 짧은 규격부터 맞춰 줄게.",
    "무게표 말고 궤적이 잘 읽히는 걸 골라 봐. 숫자는 내가 볼 테니까.",
    "반응 빠른 장비는 흔들림도 솔직해. 그걸 감수할 수 있으면 재밌는 선택이지.",
  ],
  balanced: [
    "임무랑 자세부터 말해 봐. 그 기준으로 중심을 예쁘게 잡아 보자.",
    "뭘 더 붙일지보다 끝까지 쥘 수 있는지 먼저 보자. 덤은 그다음이고.",
    "임무에서 제일 많이 할 동작이 뭐야? 그걸 기준으로 골라 줄게.",
  ],
};
...
const TEMPER_WEAPON_CHECKOUT_LINES = [
  "자, 손님. 날이랑 중심, 체결부까지 봤어. 첫 사용 뒤엔 장비도 손목도 같이 확인해.",
...
const TEMPER_ARMOR_CHECKOUT_LINES = [
  "자, 손님. 판재와 체결부 확인했고 치수도 맞췄어. 맞고 나면 방호구보다 몸부터 확인해.",
...
const TEMPER_REFERRAL_CHECKOUT_LINES = [
  "자, 손님. 검수 끝났고 치수도 맞췄어. 수리도 못 하게 박살 내서 가져오지만 마, 알겠지?",
