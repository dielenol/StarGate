...
import { canEditCharacter, canEditLore, hasRole } from "../rbac.ts";
...
test("S1-10b: legacy ObjectId ownerId도 정확히 같은 사용자면 player 모드", () => {
  const ownerId = new ObjectId();

  assert.deepEqual(
    canEditCharacter(ownerId.toHexString(), "U", character(ownerId)),
    { mode: "player", allowed: true },
  );

  assert.deepEqual(
    canEditLore(ownerId.toHexString(), "U", {
      type: "AGENT",
      ownerId,
    }),
    { mode: "player", allowed: true },
...
test("S1-12: canEditLore — 비-V owner NPC 는 player 편집 불가", () => {
  const decision = canEditLore(OWNER_ID, "U", {
    type: "NPC",
    ownerId: OWNER_ID,
  });
  assert.deepEqual(decision, {
...
test("S3-1: PLAYER_ALLOWED_CHARACTER_FIELDS — 자가편집 허용 필드 정확히", () => {
  const expected = [
    "lore.quote",
    "lore.appearance",
    "lore.personality",
...
test("S3-3: PLAYER_ALLOWED_CHARACTER_FIELDS — 식별/소유권/경제 필드 미포함", () => {
  for (const forbidden of [
    "sheet.hp",
    "sheet.san",
    "sheet.atk",
    ...
    "lore.name", // 본명도 admin 영역 — 위장 정체성 변경 차단
  ]) {
    assert.ok(
      !PLAYER_ALLOWED_CHARACTER_FIELDS.has(forbidden),
      `금지 필드 포함됨: ${forbidden}`,
...
test("S3-4: PLAYER_ALLOWED_CHARACTER_FIELDS — 루트 키 미포함 (덮어쓰기 차단)", () => {
  // lore/play 루트가 화이트리스트에 들어가면 sub-document가 통째로 $set 된다.
  // dot path 만 사용하는 게 P1 P5 의 안전 핵심.
  assert.ok(
    !PLAYER_ALLOWED_CHARACTER_FIELDS.has("lore"),
    "PLAYER 화이트리스트는 dot path 만, 루트 'lore' 키 금지",
  );
  assert.ok(
    !PLAYER_ALLOWED_CHARACTER_FIELDS.has("play"),
    "PLAYER 화이트리스트는 dot path 만, 루트 'play' 키 금지",
  );
});

test("S3-5: PLAYER_ALLOWED_CHARACTER_FIELDS — 모든 항목이 lore/play dot path", () => {
  for (const path of PLAYER_ALLOWED_CHARACTER_FIELDS) {
    assert.ok(
      path.startsWith("lore.") || path.startsWith("play."),
      `lore/play prefix 외 필드: ${path}`,
    );
  }
});

test("S3-6: CharacterEditForm.PLAYER_EDITABLE_FIELDS — derive 로직 검증", () => {
  const derivedLore = new Set(
    [...PLAYER_ALLOWED_CHARACTER_FIELDS]
      .filter((p) => p.startsWith("lore."))
      .map((p) => p.slice("lore.".length)),
  );
  const derivedPlay = new Set(
    [...PLAYER_ALLOWED_CHARACTER_FIELDS]
      .filter((p) => p.startsWith("play."))
      .map((p) => p.slice("play.".length)),
  );

  const expectedLore = new Set([
    "quote",
    "appearance",
    "personality",
    "background",
  ...
  assert.equal(derivedLore.size, expectedLore.size);
  for (const key of expectedLore) {
    assert.ok(derivedLore.has(key), `폼 헬퍼가 lore.${key} 를 잠금 해제해야 함`);
  }
  assert.equal(derivedPlay.size, expectedPlay.size);
  for (const key of expectedPlay) {
    assert.ok(derivedPlay.has(key), `폼 헬퍼가 play.${key} 를 잠금 해제해야 함`);
  }
});
