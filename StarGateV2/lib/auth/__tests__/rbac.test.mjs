...
test("S3-1: PLAYER_ALLOWED_CHARACTER_FIELDS — lore.* 8개 필드 정확히", () => {
  // P5 합의 + Review-Fix ASK A — 자가편집 가능 필드는 lore의 서사/신상 8필드.
...
test("S3-5: PLAYER_ALLOWED_CHARACTER_FIELDS — 모든 항목이 'lore.' prefix", () => {
  // CharacterEditForm 의 PLAYER_EDITABLE_FIELDS 는 .filter(p=>p.startsWith('lore.')) 후
  // .slice('lore.'.length) 로 derive — 만약 lore 외 필드(예: 'codename')가 들어가면
  // form 헬퍼가 그 필드를 누락해 server↔client drift 발생.
  for (const path of PLAYER_ALLOWED_CHARACTER_FIELDS) {
    assert.ok(
      path.startsWith("lore."),
      `'lore.' prefix 외 필드: ${path} — CharacterEditForm.PLAYER_EDITABLE_FIELDS derive 가 누락함`,
    );
  }
});
...
test("S3-6: CharacterEditForm.PLAYER_EDITABLE_FIELDS — derive 로직 검증", () => {
  ...
  assert.equal(derived.size, expected.size, "derive 결과 8개여야 함");
