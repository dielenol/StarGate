 ...
 *   - toDbNpc 변환 후 lore.posterImage 에 정확히 반영
 *   - 빈 문자열은 emptyToUndefined 정규화로 undefined 매핑
 *   - frontmatter에 posterImage 키 자체가 없으면 lore.posterImage === undefined
...
test("S6: loreSheetSchema — posterImage optional 명시", () => {
  // lore 단독 schema에서도 posterImage가 optional이어야 (없어도 통과)
  const minimalLore = {
    name: "포스터",
    nameEn: "Poster",
    mainImage: "",
  ...
  assert.doesNotThrow(() => loreSheetSchema.parse(minimalLore));
...
test("S6: toDbNpc — frontmatter에 posterImage 명시 시 lore.posterImage 동일 반영", () => {
  const doc = toDbNpc(
    {
      ...baseFrontmatter,
      posterImage: "/assets/peoples/wexler-poster.png",
  ...
  assert.equal(doc.lore.posterImage, "/assets/peoples/wexler-poster.png");

  // npcDocSchema 통과 여부도 보장
  const validated = npcDocSchema.parse(doc);
  assert.equal(validated.lore.posterImage, "/assets/peoples/wexler-poster.png");
});
...
test("S6: toDbNpc — 절대 URL posterImage 반영", () => {
  ...
  assert.equal(doc.lore.posterImage, "https://cdn.example.com/poster.png");
});

test("S6: toDbNpc — frontmatter에 posterImage 미존재 시 lore.posterImage === undefined", () => {
  const doc = toDbNpc(baseFrontmatter, emptyBody);
  assert.equal(doc.lore.posterImage, undefined);

  // npcDocSchema 통과 (optional)
  const validated = npcDocSchema.parse(doc);
  assert.equal(validated.lore.posterImage, undefined);
});
...
test("S6: toDbNpc — 빈 문자열 posterImage는 emptyToUndefined로 undefined 정규화", () => {
  ...
  assert.equal(
    doc.lore.posterImage,
    undefined,
    "빈 문자열 posterImage는 lore.posterImage === undefined로 정규화되어야 함"
  );
});
