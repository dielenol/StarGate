/** Runtime-neutral stock event rules shared by web and worker. */
...
export function rollStockMarketEvent(
  ...
  if (roll < SHOCK_CHANCE && shockEvents.length > 0) {
    const event = pickRandom(shockEvents, random);
    return {
      tier: "shock",
      text: event.text,
      percent: randomInRange(event.minPercent, event.maxPercent, random),
    };
  }

  if (roll < SHOCK_CHANCE + SCENARIO_CHANCE && scenarioEvents.length > 0) {
    const event = pickRandom(scenarioEvents, random);
    return {
      tier: "scenario",
      text: event.text,
      percent: randomInRange(event.minPercent, event.maxPercent, random),
