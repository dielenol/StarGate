...
- 운영 화면은 비로그인 게스트와 `M`·`J`·`GM` 계정으로 확인했다. 게스트 목록은 0건으로 유지됐다. 저장된 역할별 테스트 계정 6개는 `A`, `G`, `H`, `J`, `M`, `V`이며 `U` 계정은 없으므로, 공개 경계의 아래는 비로그인 게스트로, 저장 계정 중 가장 낮은 역할은 `J`로 확인했다. 따라서 `U` 계정 자체의 로그인 확인만 자료 부재로 남는다.
- `J` 화면에서는 권한에 따라 바로 보이는 참가자 링크 6건과 보고서 문맥으로 보완된 링크 12건을 합쳐 중복 없는 관련 인물 18명을 확인했다. `GM` 화면에서는 관련 인물 18명이 모두 참가자 링크로 표시되고 보조 문맥 링크는 0건이었다. 두 역할 모두 리처드와 닥터 제노가 연결됐고, `키아나 오 캘러핸 / 네베드`의 성씨 때문에 다른 오캘러핸 일가가 잘못 붙는 현상은 0건이었다.
- 최종 운영판의 보고서 이미지는 로고 1개와 장면 34개를 합쳐 35개이며 깨진 이미지는 0건이다. 장면 34개는 1035×503 비율과 `contain` 표시를 유지했고, `J`·`GM` 화면의 브라우저 오류는 0건이었다.
- 관련 인물 오연결 수정판은 운영 배포 브랜치 `589219ff`까지 푸시했고, 최종 운영 배포 `dpl_41zTnAiJMaYgREdH9g47kiUPxpaQ`가 `www.ordonet.co.kr`과 `ordonet.co.kr`에 `Ready` 상태로 연결됐다. 배포 뒤 오류 로그는 0건이었다.
