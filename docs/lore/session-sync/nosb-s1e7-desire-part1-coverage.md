...
아래 표의 `available / complete`는 원본과 등록 파일이 있고 분석 확인표가 작성됐다는 뜻이다. 운영 ERP 저장 완료를 뜻하지 않는다.
...
- 이미지: 대화용 작은 아바타와 별도로 1035×503 장면 삽화 46개가 있다. 같은 구도의 반복 8개를 제외한 서로 다른 장면 38개를 작전보고서와 같은 내용을 위키에서 찾게 하는 위키판에 같은 순서·설명으로 사용한다.
...
|---|---|---|---|---|
| 욕구 1부 전체 기록 | 142쪽 본문과 시작·종료 표기 | 작전보고서, 같은 내용을 위키에서 찾게 하는 위키판 | 보고 순번 `07`, V 이상 내부 검토자 제한 열람, 장면 38개와 관련 링크 작성 | 등록 파일·읽기 전용 dry-run 완료; 운영 ERP 미변경 |
...
| 화이트로즈 | R 단말 안내, 리처드 대면, 언론 공개 임무 | 기존 화이트로즈 위키·R 신원조회 검토안 | 대면 순서를 기록. 리처드와 R의 신원 병합은 사용자 확인 전 보류 | 신원 결정 필요 |
| 마리아의 과거 | 몽테스팡 저택·흑미사·이골로냑 의식, 카타콤 공황 | 마리아 신원조회·작전보고서 | 세션 등장과 배경·성격 관찰 추가. 이름 없는 귀족 3명은 독립 신원조회로 만들지 않음 | 전체 공개 때 추가 예정 |
| 킴라박 군부 전환 | GM의 소속 변경 확정, 웩슬러의 채용·차량 증여, 자료 전달 | 킴라박 신원조회·NHI 문서 카탈로그 | 소속은 `MILITARY / USA` 추천. 기존 MANUS·섹터 A와 내부 J 등급 처리 방식은 사용자 확인 뒤 함께 변경 | 조직·등급 결정 필요 |
| NHI 로켓·우주 엔진 적응 문서 | 소포로 킴라박에게 돌아온 뒤 웩슬러에게 전달 | 기존 카탈로그 항목 | 마지막 확인 경로를 `킴라박 → 캘빈 R. 웩슬러 부통령`으로 갱신. 인벤토리 수량은 계속 0 | 전체 공개 때 카탈로그 설명 갱신 예정 |
| 웩슬러 대통령 | 부통령의 아들이자 현직 대통령으로 직접 등장 | 신규 신원조회 검토안 | 법적 이름·전용 초상이 없으므로 독립 신원조회를 바로 만들지 않고 `웩슬러 대통령`으로 보고서에만 기록 | 후속 자료 또는 사용자 생성 승인 필요 |
...
| 웩슬러가 준 고급차 | 차 키 1개를 킴라박에게 직접 전달 | 작전보고서·킴라박 등장 기록 | 제조사·모델·차량 등록 정보가 없어 카탈로그나 장비로 만들지 않음 | 작전보고서 기록 완료; 별도 장비 없음 |
...
| 킴라박 리 | `KIMLEE` | NHI 문서 재확보·웩슬러 채용·군부 전환, 술집 잠입 뒤 부상 |
...
| 존 웡 | `JOHN_WONG` | 킴라박의 백악관 접촉을 중개 |
| 캘빈 R. 웩슬러 | `WEXLER` | 킴라박을 군부로 영입하고 고급차를 증여, NHI 자료 수령 |
...
| R | `WHITE_ROSE_R` | 해쉬를 리처드에게 안내하고 파리 카타콤 증거 공개 임무를 연결. 리처드와 동일인인지는 별도 확정 필요 |
...
|---|---|---|---|
| NHI 로켓·우주 엔진 적응 문서 1건 | 문서가 킴라박에게 소포로 돌아왔고 킴라박이 웩슬러 부통령에게 자료를 넘김 | 웩슬러 부통령이 수령한 미국 군부 자료 | 기존 카탈로그의 마지막 확인 경로만 갱신; 개인·공용 인벤토리 수량 0 유지 |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `KIMLEE` | 킴라박 리 / Kimlabak Lee — 기존 신원조회와 세션 화자 일치 | 기존 별칭 보존 | `군인 / 섹터 D 출신 현장 요원` → `미국 군부 협력자 / 전 노부스 오르도 현장 요원` 추천; GM이 소속 변경을 직접 확정 | 기존 안정 식별자와 화자명·모리어티 과거 기록 일치 | `NOVUS_ORDO / MANUS / SECTOR_A` → `MILITARY / USA`, `institutionCode` 제거 추천 | 웩슬러 부통령 직속 채용 제안을 수락하고 NHI 자료를 전달 | `J` 처리 미확정 — 외부 군부는 원칙상 등급을 저장하지 않지만 플레이어 ERP 접근·현장 참가가 계속되어 제거 영향 확인 필요 | 기존 AGENT 이미지 역할 전부 보존 | 기존 공개 보존 | 남성 35세·182cm·74kg 등 기존 확정값 보존 | 군부 전환, 웩슬러 채용·자료 전달·차량 증여와 관계를 추가해야 함 | blocked |
| `WHITE_ROSE_R` | 기존 `R`; 이번 로그의 리처드와 동일인 병합 추천 | `R`, `리처드` 추천 | 화이트로즈 수장 / 노부스 오르도 총장 보좌관 이중 신분 추천 | R 단말이 한 코너를 돌라고 안내한 직후 리처드가 대면하고 `우리 조직`의 마지막 임무를 제안 | 기존 `CIVIL / WHITE_ROSE` 정규 소속 보존 | 노부스 오르도 총장 보좌관 신분으로 잠입·겸임한 것으로 추천 | 없음 유지 — 외부 화이트로즈 인물이며 총장 보좌관 위장은 내부 등급 근거가 아님 | 기존 빈 초상 보존; 보고서 컷신은 신원조회 초상으로 전용하지 않음 | 기존 공개 보존 | 실명 리처드 외 나이·성별·신체 정보는 기존 기록 또는 원문 범위만 보존 | 해쉬에게 감염자 은폐 증거를 언론에 공개하라고 지시한 관계 추가. 신원 병합은 사용자 확인 필요 | blocked |
| `WEXLER` | 캘빈 R. 웩슬러 — 기존 신원과 세션의 `웩슬러 세니어`·부통령·전직 대통령 이력 일치 | 웩슬러 세니어 | 미국 부통령 / 외부 군부 핵심 인사 유지; 킴라박 영입자 추가 | 기존 spec의 아들 대통령·부통령 설정과 세션 부자 관계 일치 | `MILITARY / USA` 보존 | 킴라박을 직속으로 영입 | 없음 유지 — 외부 군부 인사 | 기존 `/assets/npcs/Calvin-R-Wexler-profile.webp` 보존 | 기존 공개 보존 | 기존 남성·76세·188cm·체중 미상 보존 | 돈·차량·지위로 킴라박을 영입하고 NHI 자료를 받은 사건·관계 추가 | ready-for-apply |
| `REGISTRAR` | 아그네타 스톨 — 기존 신원조회와 세션 호칭 일치 | 레지스트라 | 사무국장 / 작전 일정·지원 총괄 유지; 파리 작전 직접 지원 | 기존 안정 식별자·기관 spec·세션 화자 일치 | `NOVUS_ORDO / SECRETARIAT` 기존값 보존 | 사무총장 직속 파리 작전 지원 | `M` 유지 — 위치 안내·판정 보정·1회 보급은 기존 일정·동선·지원 총괄 범위라 접근 권한 확대 없음 | 기존 신원조회 초상 보존 | 기존 공개 보존 | 기존 신상 보존 | 파리 세 위치 안내와 현장 지원 appearance 추가 | ready-for-apply |
| `DOCTOR_ZENO` | 닥터 제노 — 기존 신원조회와 세션 화자 일치 | 제노 | 연구 기구 사무차장 / 프로젝트 데드 핸드 지휘 유지; 고문 실험 확대 선언 | 기존 안정 식별자와 S1E6 연속 등장 | `NOVUS_ORDO / SECRETARIAT / RESEARCH` 보존 | 마가렛·우디와 성과 미달 능력자 통제 | `V` 유지 — 실험 방식과 통제 범위가 확대됐지만 연구 기구 사무차장·프로젝트 지휘 직책과 접근 범위 자체는 변하지 않음 | 기존 공용 미확인 초상 보존 | 기존 공개 보존 | 기존 미상 인적 정보 보존 | 마가렛 고문 실험, 우디 외출 금지, 오틸리아와의 적대 관계·성격 관찰 추가 | ready-for-apply |
| `MARGARET` | 마가렛 — 기존 신원조회와 세션 대상 일치 | 메리골드 등 기존 별칭 보존 | 프로젝트 데드 핸드 실험 대상 유지 | 기존 안정 식별자·제노 보고 | 기존 소속·배치 보존 | 하루 16시간 고문 모의실험 대상 | 기존 등급 유지 — 피해·능력 변화는 직책·접근 권한 변화가 아님 | 기존 AGENT/NPC 이미지 역할 보존 | 기존 공개 보존 | 기존 신상 보존 | 송과선 활성화와 강령 능력 증대 보고는 제노의 관측으로 기록 | ready-for-apply |
| `WATER_FIST_RACKY` | `"물주먹" 래키` / Water-Fist Racky — 기존 신원조회와 세션 화자 일치 | 물주먹 | 송사리 탐정 유지; 페데라치오 추적 정보원·현장 합류 | 기존 안정 식별자와 `송사리 선박` 언급, 능력 사용 일치 | `CIVIL / SONGSARI` 보존 | 파리 길목팀 임시 동행 | 없음 유지 — 외부 시민사회 인물 | 기존 `/assets/npcs/Water-Fist-Racky-profile.webp` 보존 | 기존 공개 보존 | 기존 남성·44세·177cm·체중 미상 보존 | 페데라치오에게 구타당한 뒤 변호사 정보를 제공하고 현장팀과 합류한 appearance 추가 | ready-for-apply |
| `JOHN_WONG` | 존 웡 — 기존 신원조회와 `CIA 존 웡` 화자 일치 | 기존 별칭 보존 | 미국 정보기관 접촉자 유지 | 기존 안정 식별자와 킴라박·모리어티 접점 연속성 | `MILITARY / USA` 보존 | 킴라박의 백악관 접촉 중개 | 없음 유지 — 외부 군부 인물 | 기존 초상 보존 | 기존 공개 보존 | 기존 신상 보존 | 킴라박과 웩슬러 사이의 연락·안내 관계 추가 | ready-for-apply |
| `AMALIA_FREDRIKA_VON_ESSEN` | 아말리아 프레드리카 본 에센 — 기존 사무총장 신원과 화자 직함 일치 | 사무총장 | 페데라치오 소탕 작전 지휘 유지 | 기존 안정 식별자와 직함·지휘 범위 일치 | 기존 `NOVUS_ORDO / SECRETARIAT` 보존 | 파리 비밀 작전 직접 지휘 | 기존 등급 유지 — 소탕 명령은 사무총장의 기존 권한 범위 | 기존 초상 보존 | 기존 공개 보존 | 기존 신상 보존 | 프랑스 정부 배제·소탕 우선 지시와 왕관 백신 설명 appearance 추가 | ready-for-apply |
| `MR_ODD` | Mr. 오드 — 기존 신원조회와 세션 화자 일치 | 기존 별칭 보존 | 블랙 피라미드 운영·현장 통제 역할 유지 | 기존 안정 식별자와 격리 사고 보고 역할 일치 | 기존 소속·배치 보존 | ZULU-0060·0007 격리 붕괴 피해 보고 | 기존 등급 유지 — 사고 수습 보고는 기존 운용 범위 | 기존 초상 보존 | 기존 공개 보존 | 기존 신상 보존 | 30명 미만 사상자 평가와 구역 정리 appearance 추가 | ready-for-apply |

## Story-Driven Role/Level Review

- `KIMLEE`: GM은 `킴라박 리의 소속은 이제 군부로 바뀝니다`라고 직접 확정했다. 따라서 소속 변경 자체는 로그 확정이다. 구조화 추천은 `factionCode: MILITARY`, `department: USA`, `institutionCode` 제거다. 다만 외부 인물은 내부 `agentLevel`을 두지 않는 규칙과 플레이어 캐릭터의 ERP 접근·현장 참가가 충돌할 수 있어, `J`를 제거할지 운영상 유지할지 사용자 결정을 받아야 한다. 결정 전에는 소속 필드를 변경하지 않는다.
- `WHITE_ROSE_R`: 역할이 바뀐 것이 아니라 기존 R 신원과 총장 보좌관 리처드 신원을 병합하는 검토다. 기존 `CIVIL / WHITE_ROSE`, 외부 무등급을 유지하고 `리처드`를 실명·노부스 오르도 보좌관 위장 신분으로 추가하는 안을 추천한다. 신원 병합은 사용자 확정 전 실행하지 않는다.
...
| `NOSB-S1E7-DESIRE-PART1:KIMLEE:transactional-hierarchical-loyalty` | `KIMLEE` | `NOSB-S1E7-DESIRE-PART1` | 명확한 지휘 체계와 보상에 자신을 맡기는 거래적 충성 | description + dialogue + action | 명령에 따르는 단순한 체계를 편하게 여긴다고 생각하고, 웩슬러 앞에 무릎 꿇어 쓰임에 맞게 거둬 달라며 자료와 계좌번호를 즉시 넘김 | 작전 보고서 S1E7: 욕구 1부 | confirmed | candidate-only |
...
| `NOSB-S1E7-DESIRE-PART1:WEXLER:patronage-through-reward` | `WEXLER` | `NOSB-S1E7-DESIRE-PART1` | 돈·차량·지위로 충성을 사는 후견 정치 | dialogue + action | 킴라박에게 주급 인상과 고급차를 제시하고 대통령을 위해 일하라고 권한 뒤 NHI 자료를 받음 | 작전 보고서 S1E7: 욕구 1부 | confirmed | ready-for-apply |
| `NOSB-S1E7-DESIRE-PART1:WHITE_ROSE_R:strategic-exposure` | `WHITE_ROSE_R` | `NOSB-S1E7-DESIRE-PART1` | 여론과 내부 증거로 조직 정책을 바꾸려는 전략적 폭로 | dialogue + action | R의 단말 안내 직후 리처드가 파리 카타콤 감염자 증거를 확보해 언론에 공개하라고 지시함 | 작전 보고서 S1E7: 욕구 1부 | candidate | candidate-only |
...
| `KIMLEE` | `WEXLER` | 차량·급여 제안을 받고 충성을 맹세하며 자료 전달 | 군부 소속 결정과 함께 반영 대기 |
| `WEXLER` | `KIMLEE` | 영입·후견·자료 수령 | 세션 관계 추가 예정 |
| `JOHN_WONG` | `KIMLEE` | 백악관 접촉을 중개 | 세션 관계 추가 예정 |
| `WATER_FIST_RACKY` | `INDEXER` | 구조 뒤 페데라치오 정보를 제공하고 길목팀에 합류 | 임시 협력 관계 추가 예정 |
| `WHITE_ROSE_R` | `INDEXER` | 파리 감염자 증거 공개 임무를 지시 | 리처드 신원 병합 결정 뒤 반영 |

## Visual Asset Ledger

보고서 시각자료는 PDF 안의 가로형 1035×503 장면 전체를 추가로 자르지 않고 WebP로 바꾼다. 같은 장면을 보여 주는 반복 8개는 먼저 나온 프레임을 남기고 제외한다. 작전보고서와 위키판은 로고 1개와 장면 38개의 경로·순서·대체 설명·캡션을 정확히 같게 유지한다. 장면 컷신은 신원조회 초상, 카탈로그 그림, 개체 위키 대표 이미지로 자동 재사용하지 않는다.
...
| `/assets/session-reports/s1e7-desire-part1/kimlabak-off-duty-nhi-package.webp` | PDF p039 X830 | 1035×503 | no — full PDF XObject | report-cutscene | included: 비공개 보고서 등록 파일에 포함 | included: 보고서와 동일 순서·설명 | excluded: 보고서 장면으로만 사용 | excluded: 독립 아이템 도판이 아님 | excluded: 신원조회 전용 초상이 아님 | NHI 문서가 킴라박에게 돌아온 사건 |
| `/assets/session-reports/s1e7-desire-part1/kimlabak-arrives-white-house.webp` | PDF p042 X893 | 1035×503 | no — full PDF XObject | report-cutscene | included: 비공개 보고서 등록 파일에 포함 | included: 보고서와 동일 순서·설명 | excluded: 보고서 장면으로만 사용 | excluded: 독립 아이템 도판이 아님 | excluded: 신원조회 전용 초상이 아님 | 킴라박의 백악관 도착 |
| `/assets/session-reports/s1e7-desire-part1/white-house-neocon-gathering.webp` | PDF p043 X916 | 1035×503 | no — full PDF XObject | report-cutscene | included: 비공개 보고서 등록 파일에 포함 | included: 보고서와 동일 순서·설명 | excluded: 보고서 장면으로만 사용 | excluded: 독립 아이템 도판이 아님 | excluded: 신원조회 전용 초상이 아님 | 웩슬러 일가와 미국 매파 모임 |
| `/assets/session-reports/s1e7-desire-part1/wexler-recruits-kimlabak.webp` | PDF p045 X961 | 1035×503 | no — full PDF XObject | report-cutscene | included: 비공개 보고서 등록 파일에 포함 | included: 보고서와 동일 순서·설명 | excluded: 보고서 장면으로만 사용 | excluded: 독립 아이템 도판이 아님 | excluded: 신원조회 전용 초상이 아님 | 웩슬러 부통령의 킴라박 영입 |
...
|---|---|---|---|
| 킴라박의 군부 전환 | GM이 소속 변경을 직접 선언했고, 킴라박은 웩슬러의 채용을 수락해 NHI 자료를 넘겼다. | `MILITARY / USA`, MANUS·섹터 A 해제는 근거가 충분하다. | 내부 `J` 등급을 제거하면 플레이어 ERP 접근에 영향이 있을 수 있어 제거 또는 운영상 유지 중 하나를 사용자에게 확인한다. |
| R과 리처드 | R 단말의 이동 지시 직후 리처드가 나타나 `우리 조직`의 마지막 임무를 제안했다. | 동일인일 가능성이 매우 높아 기존 `WHITE_ROSE_R`에 리처드를 병합하는 안을 추천한다. | 사용자가 동일인 병합을 확정하기 전에는 신원조회 값을 바꾸지 않는다. |
| 웩슬러 대통령 | 현직 대통령이며 캘빈 R. 웩슬러의 아들로 등장했다. | 보고서에는 직함과 부자 관계만 기록한다. | 실명이나 전용 초상이 확인될 때 독립 신원조회를 검토한다. |
| NHI 문서 보관 경로 | 킴라박에게 돌아온 문서를 웩슬러 부통령이 수령했다. | 기존 공개 카탈로그의 마지막 확인 경로를 갱신할 근거가 충분하다. | 기존 공개 문서가 새 회차의 비공개 내용을 먼저 노출하지 않도록 전체 공개 시점에 함께 갱신한다. |
...
- 보고서와 위키판은 로고 1개와 장면 38개의 경로·순서·대체 설명·캡션이 정확히 같아야 한다.
- 장면 WebP 38개는 모두 1035×503이어야 하며, 반복 8개는 공개 자산으로 생성하지 않는다.
...
- `KIMLEE`와 `WHITE_ROSE_R`의 기존 신원·소속·등급은 이번 비공개 등록 파일에서 변경하지 않는다.
- 운영 ERP 저장을 승인받아 실행한 뒤에는 같은 기록을 다시 읽어 실제 저장값이 등록 파일과 같은지 비교하고, GM 및 V 화면에서 보고서·위키·이미지·상호 링크를 확인해야 한다.
- 전체 공개는 두 신원 결정과 문서 다듬기가 끝난 뒤 별도 등록 파일로 수행하고, 그때 기존 공개 위키·카탈로그·신원조회의 새 사건 연결도 함께 갱신한다.

## 실제 운영 ERP 반영 상태

- 이번 작업은 로컬 커밋까지만 진행하며, push·배포·운영 DB 저장은 하지 않는다.
- 이 확인표와 비공개 등록 파일, 정적 장면 자산은 로컬 저장소에만 준비한다.
- 현재 운영 ERP/DB에는 이 회차의 새 보고서·위키·신원·아이템 변경을 쓰지 않았다.
- `public/assets/session-reports/s1e7-desire-part1/`의 장면 38개는 배포되는 즉시 주소를 아는 사람이 직접 열 수 있으므로, 현재 비공개 보고서만 운영 DB에 저장하거나 이미지 파일만 먼저 배포해서는 안 된다.
- 문서 다듬기와 신원 결정을 끝낸 뒤 전체 공개와 이미지를 동시에 배포하거나, V 이상만 이미지를 읽을 수 있는 보호 경로를 마련한 경우에만 운영 반영을 재개한다.
