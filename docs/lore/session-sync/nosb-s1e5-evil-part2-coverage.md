...
| 러시아 장군 미하일 게라쉬모프 | p.58-p.60, p.92-p.97 + 사용자 제공 초상 + 후속 신원 확인 | 기존 NPC spec + Dossier, report 1·2부 | 전체 이름과 러시아 정부 분류에 맞춰 identity·organization prose를 동기화하고, 오틸리아·로드리온 관계와 사망은 보존 | applied |
...
| 게라쉬모프 신원 | 작전 기록의 성·장군 직함·러시아 측 역할·초상과 사용자 후속 전체 이름 확인 | `GERASIMOV`, 표시명 `미하일 게라쉬모프`, `MILITARY / RUSSIA`, 등급 없음, 공개·사망·초상 보존 | 없음 — identity·organization prose focused repair와 DB 재조회 완료 |
...
- 2026-08-10 `GERASIMOV` spec과 신규 생성 원본 payload의 adapter-managed fields를 전체 이름·`department: RUSSIA` 기준으로 다시 맞췄다. focused repair는 기존 표시명·영문명·외형·배경·역할 상세·이름 설명·`loreMd` 부재까지 exact CAS로 보호하고, 목표 표시명·영문명·department·관련 공개 서술·`loreMd`·태그를 함께 썼다. live `stargate.characters`의 `_id=6a7587a61bc9225fc2395022`는 `미하일 게라쉬모프`, `Mikhail Gerasimov`, `MILITARY / RUSSIA`, 공개·사망·외부 무등급·기존 초상·`loreMd`·`러시아정부` 태그로 재조회됐고 기존 event 2·session appearance 2·relation 2는 유지됐다. `lore_ingestion_runs.runId=seed-payload:3d801ed8-7702-4637-aee4-adee8017d9e6`은 `succeeded`이며 사후 dry-run은 `예상 unchanged`였다.
