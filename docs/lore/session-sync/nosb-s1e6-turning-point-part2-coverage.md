...
- 시각 자료: PDF 안에는 `1035×503` 크기의 가로형 장면 삽화 25개가 있다. 이전 답변의 `TIER-3 장면 프레임`은 바로 이 삽화를 뜻한다. 작은 대화 아바타 1,492개는 신원조회용 단독 초상이 아니다. 반복 구도 7개를 제외한 장면 18개를 PDF 안의 전체 장면 그대로 추출해 작전보고서와 같은 보고서의 위키판에 같은 순서·설명으로 배치했다. 장면 18개·로고·신원조회 초상 2개·카탈로그 이미지 4개는 커밋 `6f07257c`로 원격 배포됐고, 운영 도메인에서 25개 파일 모두 HTTP 200과 WebP 응답을 확인했다.
- 공개 흐름: 원본 표지에는 `CLASSIFICATION · TIER-3 · EYES ONLY`가 적혀 있다. 사용자 승인에 따라 `2026-08-26` 작전보고서·보고서 위키판·잔광자 위키·모리어티·광체 소녀 신원조회를 먼저 비공개로 저장해 GM 화면을 확인한 뒤, 같은 승인 범위에서 보고서 `minRole: U`와 관련 위키·신원조회·카탈로그를 전체 공개했다. 기존 위키 7건과 기존 신원조회 21명의 등장 기록·관계·성격 관찰도 같은 공개 작업에서 연결했다.
...
- `비공개 초안 저장`: 기록을 일반 이용자에게 공개하지 않은 상태로 ERP에 저장해 검토하는 일이다. 이번 작업에서는 신원조회 2건·위키 2건·작전보고서 1건을 한 트랜잭션으로 비공개 저장하고, DB 재조회와 GM 화면 확인까지 마친 뒤 전체 공개했다.
...
- 공개 기준: 다듬는 동안 전부 비공개로 유지하고, 사용자가 완성됐다고 확인하면 전체 공개한다. 카탈로그 4건과 승인된 인벤토리 수량, 작전보고서·위키·신원조회·기존 문서 연결은 실행 범위를 제시해 승인받은 뒤 `2026-08-26`에 모두 반영·재조회·화면 확인을 마쳤다.
...
|---|---|---|---|---|
| 변곡점 2부 전체 기록 | 141쪽 전 구간과 본문 시작·종료 표기 | 작전보고서, 같은 내용의 위키판 | 보고 순번 `06.5`로 본문·관련 링크 작성. 장면 18개를 두 화면에 같은 순서·설명으로 배치한다. 비공개 신원조회 링크까지 끊김 없이 확인할 수 있도록 다듬기 단계에서는 GM만 볼 수 있게 준비 | 비공개 저장·GM 확인 뒤 전체 공개 완료; DB·GM·일반 계정·게스트 화면 재확인 완료 |
| 보고서 번호·지도 카드 | S1E6 2부와 1부의 수메르 권역이 같음 | 보고서 목록·지도 | `06.5`가 1부의 `06` 카드와 겹치지 않도록 표시 | 운영 화면에서 `06.5` 확인 완료 |
| 아르고 기지 NHI 기록 | NHI 우주 엔진 문서, 하이퍼보리아인 14, Montag-98, 기계 하이에나 | 기존 몬탁 프로젝트 위키, 작전보고서 | 별도 과거 기록으로 추가하되 Montag-98을 기존 도살견과 같은 개체로 단정하지 않음 | 공개 반영·재조회 완료 |
| 모리어티 대령 | 섹터 D 대령, 킴라박의 직속 상관, 문서 탈취·부하 살해 시도, Montag-98에게 사망 | 신규 모리어티 신원조회, 작전보고서, 기존 인물 관계 | `NOVUS_ORDO / MANUS / SECTOR_D`, `사망 확인`, 확인일 `2026-08-09`, 전용 투명 초상을 반영했다. 같은 대령·현장 지휘관 근거로 제안한 권한등급 `H`를 사용자가 `2026-08-26`에 확정했다. | 전체 공개 완료; 섹터 D·H·사망·초상·보고서 연결을 DB와 운영 화면에서 확인 |
| 스페이스 제로 주주의 방주 | 텍사스 로켓, 부유체 요구, 서면 계약 | 기존 스페이스 제로 위키, 스타크·요한 신원조회, 작전보고서 | 계약과 로켓 계획만 추가하고 실제 주식·자산 이전으로 단정하지 않음 | 공개 반영·재조회 완료 |
| 뒤집어진 양말 | 로켓 부유체 요구와 발사대 사체 | 기존 뒤집어진 양말 위키, 작전보고서 | 요구와 현장 발견만 추가. 인도·소유권 변화는 미확정 | 공개 반영·재조회 완료 |
| 엘 볼라도르 편입 | 폭탄 칩셋 강제 주입, `ACCEL` 코드명, 첫 현장 증원 | ACCEL·Mr. 오드 신원조회, 작전보고서 | 등장 기록·강압 관계·성격 관찰 추가 | 공개 반영·재조회 완료 |
| 마지막 대대 | 의식장 주변 잔존 보병·중기관총병 | 기존 마지막 대대 위키, 작전보고서 | 2부 교전을 추가하되 조직 전체가 소탕됐다고 단정하지 않음 | 공개 반영·재조회 완료 |
| 광명회와 이르마 코흐 | 시유 회유, 자가 주사, 잔광자 전환·사망 | 기존 광명회 위키, 이르마·관련 인물 신원조회, 작전보고서 | 등장 기록·관계·성격 관찰과 구조화 사망 상태 `DECEASED`를 추가한다. 세션에서 사망이 확정된 시각은 `2026-08-10 01:06`이며, 신원조회 상태 확인일은 `2026-08-10`, 근거 사건은 `NOSB-S1E6-TURNING-POINT-PART2`로 묶는다. 기존 생사 상태가 비어 있거나 이 값과 정확히 같을 때만 처리하고 다른 값이면 중단한다. | 공개 반영·재조회 완료; `DECEASED / 2026-08-10 / 근거 세션` 일치 확인 |
| 잔광자 | 광채 신체, 다수의 눈, 정신 공격, 냉기·전기·화염 반응, 참수 | 신규 잔광자 위키, 작전보고서 | 이르마에게서 확인된 단일 변환 사례로만 문서 작성 | 전체 공개·화면 확인 완료 |
| 광원화 바이러스 | 개조 주사, 이르마 변환, 사백신·행동교정물질 반응 | 기존 광원화 바이러스 위키, 작전보고서 | 실제 관측과 연구 가설을 나눠 추가 | 공개 반영·재조회 완료 |
| 잔광자 광채 표본 | 피펫이 1개를 직접 채취하고 전방 수호대에게 4개를 받음 | 신규 비판매 카탈로그 항목, 피펫 개인 인벤토리, 공용 인벤토리, 작전보고서 | 사용자 지시에 따라 직접 채취한 1개는 피펫에게, 전방 수호대가 전달한 4개는 공용 인벤토리에 귀속. 같은 항목을 두 곳에서 수량만 나눠 관리 | 카탈로그 전체 공개·재조회 완료; 피펫 1개·공용 4개 유지 |
| 잔광자 머리 | 키아나가 참수하고 해쉬가 회수를 지시했으며 피펫이 회수하고 우디가 운반을 도움 | 신규 비판매 카탈로그 항목, 공용 인벤토리, 작전보고서 | 사용자 지시에 따라 생체 표본 1개를 공용 인벤토리에 귀속 | 카탈로그 전체 공개·재조회 완료; 공용 1개 유지 |
| 광체 소녀 | 이르마 곁에서 시유를 회유하고 주사기를 돌려줌, 자신을 `광체`라고 소개, 마리아의 동행 제안을 수락하고 사무총장이 연구동으로 데려가기로 결정 | 신규 광체 소녀 신원조회, 작전보고서, 잔광자 위키 | 사용자 지정 대표 표기 `광체 소녀`와 제공 초상을 사용한다. `광체`가 고유명인지 존재 분류인지는 미확정으로 남기고, 정규 소속·내부 권한등급은 만들지 않는다. 연구동 이송은 결정만 기록하고 실제 도착은 단정하지 않음 | 전체 공개 완료; 초상·연구동 이송 결정·보고서 연결을 운영 화면에서 확인 |
| 프로젝트 데드 핸드 | 마가렛 격리실 면회, 피펫의 구출 약속, 제노 통제 지속 | 기존 프로젝트 데드 핸드 위키와 세 사람의 신원조회 | 등장 기록과 관계 추가 | 공개 반영·재조회 완료 |
| 페데라치오 | 사무총장의 후속 범죄조직 브리핑 | 작전보고서 | 정확한 철자·구조가 부족해 후속 단서로만 기록 | 별도 조직 위키는 만들지 않음 |
| NHI 우주 엔진 문서 | 킴라박이 1건을 회수해 모리어티에게 전달했고, 모리어티 사망 뒤 행방 불명 | 신규 비판매 카탈로그 항목, 작전보고서 | 사용자 지시에 따라 문서의 존재·내용·마지막 확인 경로만 카탈로그에 기록. 현재 보관자가 없으므로 개인·공용 인벤토리 수량은 모두 0으로 유지 | 카탈로그 전체 공개·재조회 완료; 개인·공용 인벤토리 0 유지 |
| 이르마가 사용한 주사기 | 이르마가 자가 투여한 뒤, 마리아가 사용된 주사기 1개를 챙김 | 신규 비판매 카탈로그 항목, 공용 인벤토리, 작전보고서 | 마리아를 마지막 현장 보유자로 기록하고, 사용자 지시에 따라 사용된 현장 물증 1개를 공용 인벤토리에 귀속. 소비품이나 재사용 가능한 주사기로 취급하지 않음 | 카탈로그 전체 공개·재조회 완료; 공용 1개 유지 |
| 엘 볼라도르 폭탄 칩셋 | 목에 강제 주입, 명령 위반 시 즉결처형 언급 | 작전보고서, ACCEL·Mr. 오드 관계 | 생체 삽입 상태를 서술하고 별도 장비나 보유품으로 만들지 않음 | 처리 판단 끝남 |
| 작전 크레딧 1,000 | 마리아가 전투원 2명 호출에 1,000 사용을 선언했고 실제 합류 확인 | 작전보고서 | 세션 당시 자동 차감 흔적은 없지만 GM이 잔액을 직접 조정했는지는 남은 기록만으로 확정할 수 없다. 중복 차감 위험 때문에 새로 차감하지 않고, 세션에서 사용했다는 사실만 보고서에 기록 | 공개 보고서에 사실만 기록; 크레딧 추가 차감 없음 |

## 신원조회 등장 사건 연결

| 세션 속 이름 | 연결할 신원조회 | 추가할 등장 사건 | 현재 상태 |
|---|---|---|---|
| 킴라박 리 | `KIMLEE` | 아르고 기지 문서 회수, 모리어티 배신 생존, 수메르 잔광자 교전 | 공개 반영·재조회 완료 |
| 츠키시로 쿠즈하 / 유회 | `YUHOE` | 아르고 침투 지휘와 기계 하이에나 추격·지원 요청 | 공개 반영·재조회 완료 |
| CIA 존 웡 | `JOHN_WONG` | 섹터 D 해상 플랫폼 과거 기록 cameo | 공개 반영·재조회 완료 |
| 스페이스 제로 CEO | `JOHAN_SMITH` | 주주의 방주 로켓과 뒤집어진 양말 확보 계약 제안 | 공개 반영·재조회 완료 |
| 스타크 일로니손 | `CLOWN` | 서면 계약 체결, 잔광자 생포·연구 주장 | 공개 반영·재조회 완료 |
| Mr. 오드 | `MR_ODD` | ACCEL 강제 편입과 잔광자 파괴 명령 전달 | 공개 반영·재조회 완료 |
| 엘 볼라도르 | `ACCEL` | 폭탄 칩셋 편입과 코드명 사용, 수메르 첫 증원·교전 | 공개 반영·재조회 완료 |
| 해쉬 테거 | `INDEXER` | 사백신 사용, 잔광자 대응·표본 회수 지시 | 공개 반영·재조회 완료 |
| 휘트모어 핀치 | `PIPETTE` | 잔광자 분석·광채 5개와 머리 회수, 마가렛 방문 | 공개 반영·재조회 완료 |
| 마리아 | `MARIA` | 광체 보호, 이르마 주사기 확보, 잔광자 처리 협상 | 공개 반영·재조회 완료 |
| 오틸리아 발트만 | `OTILIA` | 행동교정물질 전달에 전 자원 소모, 생포 요구 | 공개 반영·재조회 완료 |
| 시유 | `TIGER298` | 이르마의 회유를 거부하고 섹터 동료를 선택, 잔광자 교전 | 공개 반영·재조회 완료 |
| 키아나 오 캘러핸 | `네베드` | 이사회 파괴 명령을 근거로 잔광자 참수 | 공개 반영·재조회 완료 |
| 우디 | `WD-(𝓃)` | 잔광자 교전과 머리 회수 지원 | 공개 반영·재조회 완료 |
| 이동식 | `LEE DONGSIK` | 잔광자 정신 공격과 전투 중 시스템 다운·재가동 | 공개 반영·재조회 완료 |
| 크로노스 | `TIME` | 잔광자 정신 공격·감염 대응과 위험한 포스코어 미래 확인 | 공개 반영·재조회 완료 |
| 운연 | `UNYEON` | 교전 회복 지원과 잔향 이후 발화 상실 | 공개 반영·재조회 완료 |
| 사무총장 | `AMALIA_FREDRIKA_VON_ESSEN` | 잔광자 시체 연구와 광체 연구동 이송 결정, 백신 연구 가능성 브리핑 | 공개 반영·재조회 완료 |
| 이르마 코흐 | `IRMA_KOCH` | 개조 바이러스 자가 투여, 잔광자 전환과 사망 | 공개 반영·재조회 완료; 구조화 사망 상태 일치 |
| 마가렛 | `MARGARET` | 순백 격리실 면회와 피펫 이탈 뒤 불안·좌절 | 공개 반영·재조회 완료 |
| Dr. 제노 | `DOCTOR_ZENO` | 면회 종료와 마가렛 단독 격리 지속 | 공개 반영·재조회 완료 |
| 모리어티 대령 | `MORIARTY` | 섹터 D 침투부대 지휘, NHI 문서 탈취, 킴라박 배신, Montag-98에게 사망 | 전체 공개·재조회 완료; 섹터 D·전용 초상·권한등급 `H`·사망 상태 확인 |
| 광체 소녀 | `GWANGCHE_GIRL` | 시유 회유, 이르마 주사기 반환, 마리아 보호, 연구동 이송 결정. 실제 도착은 확인되지 않음 | 전체 공개·재조회 완료; 화면 표기 `광체 소녀`와 전용 초상 확인 |
...
|---|---|---|---|---|---|
| `CLOWN` | `JOHAN_SMITH` | 뒤집어진 양말과 로켓의 부유체를 교환 조건으로 삼은 서면 계약 | 확정 | 양측 신원조회 relation | 양방향 공개 반영·재조회 완료 |
| `ACCEL` | `MR_ODD` | 폭탄 칩셋을 통한 강제 편입과 즉결처형 통제 | 확정 | 양측 신원조회 relation | 양방향 공개 반영·재조회 완료 |
| `TIGER298` | `IRMA_KOCH` | 어머니 환영과 천사 제안을 거부하고 섹터 동료에게 복귀 | 확정 | 양측 신원조회 relation | 양방향 공개 반영·재조회 완료 |
| `OTILIA` | `IRMA_KOCH` | 제자를 생포하려 전 자원을 소모했으나 이사회 명령에 따른 참수를 목격 | 확정 | 양측 신원조회 relation | 양방향 공개 반영·재조회 완료 |
| `네베드` | `IRMA_KOCH` | 위험 개체 파괴와 이사회 명령을 근거로 변환된 이르마를 참수 | 확정 | 양측 신원조회 relation | 양방향 공개 반영·재조회 완료 |
| `PIPETTE` | `MARGARET` | 반복 방문과 닥터 제노에게서 구출하겠다는 약속 | 확정 | 양측 신원조회 relation | 양방향 공개 반영·재조회 완료 |
| `DOCTOR_ZENO` | `MARGARET` | 면회 종료 뒤 마가렛을 순백 격리실에 홀로 남김 | 확정 | 양측 신원조회 relation | 양방향 공개 반영·재조회 완료 |
| `KIMLEE` | `MORIARTY` | 충성하던 지휘관에게 문서를 넘긴 뒤 살해 대상이 됨 | 확정 | 양쪽 신원조회 관계 | 양방향 공개 반영·재조회 완료 |
| `YUHOE` | `MORIARTY` | 기계형 하이에나의 추격 중 지원을 요청했지만 모리어티가 무전을 끊고 철수를 명령 | 확정 | 양쪽 신원조회 관계 | 양방향 공개 반영·재조회 완료 |
| `JOHN_WONG` | `MORIARTY` | 섹터 D 해상 플랫폼에서 악수하고 재회를 기약했으나 구체적인 협력 관계는 확인되지 않음 | 확정된 접촉 / 관계 성격 미확정 | 양쪽 신원조회 관계에 확인된 접촉 범위만 기록 | 양방향 공개 반영·재조회 완료 |
| `MARIA` | `GWANGCHE_GIRL` | 소녀를 보호하고 자신을 어머니로 호명하며 노부스 오르도로 함께 가자고 제안했고, 소녀가 고개를 끄덕여 수락 | 확정 | 양쪽 신원조회 관계 | 양방향 공개 반영·재조회 완료 |
| `IRMA_KOCH` | `GWANGCHE_GIRL` | 소녀가 이르마를 교주님이라고 부르고 주사기 반환·시유 설득에 협력 | 확정 | 양쪽 신원조회 관계 | 양방향 공개 반영·재조회 완료 |
| `TIGER298` | `GWANGCHE_GIRL` | 소녀의 천사화 설득을 거부하고 섹터 동료에게 복귀 | 확정 | 양쪽 신원조회 관계 | 양방향 공개 반영·재조회 완료 |
| `OTILIA` | `GWANGCHE_GIRL` | 자신의 유전자로 만들어졌을 가능성을 제기 | 현장 증언 — 독립 검증 없음 | 양쪽 신원조회 관계에 추정임을 명시 | 양방향 공개 반영·재조회 완료 |
...
| 대상 | 수량 | 누가 어떻게 확보했나 | 마지막으로 확인된 상태 | 개인 소유인가 | ERP 처리 결과 | 확인 결과·남은 정보 |
|---|---:|---|---|---|---|---|
| 잔광자 광채 표본 | 5개 | 피펫이 잔광자에게서 1개를 직접 채취했고, 전방 수호대가 4개를 추가로 피펫에게 전달 | 세션 후반 피펫이 5개를 휴대. 스타크에게는 안전하게 가공한 뒤 나누겠다고만 했고 실제 전달은 없음 | 직접 채취한 1개만 사용자 지시로 피펫 보유 처리. 나머지 4개는 공용 연구 자산 | 공개·비판매·지급 불가 카탈로그 1건, 피펫 개인 인벤토리 `0→1`, 공용 인벤토리 `0→4` | `2026-08-26` 공개 뒤 DB 재조회와 ERP 화면에서 피펫 1개·공용 4개 유지 확인 |
| 잔광자 머리 | 1개 | 키아나가 참수했고, 해쉬의 지시로 피펫이 회수했으며 우디가 운반을 도움 | 피펫·우디의 회수 뒤 사무총장이 시체를 백신 연구에 쓰겠다고 설명 | 아님. 피펫이나 우디의 개인 전리품이 아니라 공용 연구 표본 | 공개·비판매·지급 불가 카탈로그, 공용 인벤토리 `0→1` | `2026-08-26` 공개 뒤 DB 재조회와 ERP 화면에서 공용 1개 유지 확인 |
| NHI 로켓 추진·우주 엔진 적응 문서 | 1건 | 킴라박이 아르고 기지에서 획득해 모리어티에게 전달 | 문서를 받은 모리어티가 곧 사망했고 이후 문서 회수 기록 없음 | 누구의 소유라고도 확정할 수 없음 | 공개·비판매·지급 불가 카탈로그에 존재와 마지막 확인 경로만 등록. 개인·공용 인벤토리는 모두 `0` 유지 | `2026-08-26` 공개 뒤 카탈로그와 인벤토리 0 유지 확인. 후속 회수자·현재 보관처는 세계관상 미확정 |
| 이르마가 사용한 주사기 | 1개 | 이르마가 자신에게 사용한 뒤, 마리아가 사용된 주사기를 옷 안에 챙김 | 마리아가 마지막 현장 보유자 | 아님. 개인 소유물이 아니라 사용 후 현장 물증 | 사용 흔적이 보이는 공개·비판매·지급 불가 카탈로그, 공용 인벤토리 `0→1` | `2026-08-26` 공개 뒤 DB 재조회와 ERP 화면에서 공용 1개 유지 확인. 잔류 성분과 보관 조건은 후속 연구 정보로 남김 |
...
## NPC Approval Ledger (적용 전 승인 보존본)

이 표는 신원조회 필드를 빠짐없이 검사하기 위해 실제 적용 전에 확정한 내부 원장 원문이다. `ready-for-apply`는 당시 `근거와 등록 자료가 준비됨`을 뜻한다. 표의 준비 시점 상태명은 승인 근거 보존을 위해 바꾸지 않았으며, 실제 적용 결과는 아래 `실제 운영 ERP 반영 현황`에서 관리한다. `2026-08-26` 현재 신규 신원조회 2건과 기존 신원조회 21명의 등장 기록, 관계 21개, 성격 관찰 8개, 이르마 사망 상태까지 적용·재조회가 끝났다.
...
| `IRMA_KOCH` | 이르마 코흐 | 없음 — 광명회 수장은 직함 | 아넨에르베 광명회 수장 유지; 잔광자 변환 뒤 사망 | 화자명·1부 기존 신원조회와 연속 | 기존 외부 `AHNENERBE` 소속 보존 | 수메르 의식장 자가 주사·변환 | 외부 NPC: agentLevel 없음; 변환·사망은 사무국 권한등급 변경 대상 아님 | 기존 사용자 제공 신원조회 초상 보존; 잔광자 컷 재사용 없음 | 기존 `isPublic: true` 보존 | 기존 신상 보존. 적용 전에는 구조화 생사 상태가 없었고, 전체 공개 작업에서 `DECEASED / 2026-08-10 / NOSB-S1E6-TURNING-POINT-PART2`를 한 묶음으로 추가 완료 | OTILIA·TIGER298·네베드 관계와 관찰 누적 | ready-for-apply |
...
| `MORIARTY` | 모리어티 — 기존 캐논·사용자 표기를 대표 이름으로 유지하고, 이번 PDF의 `모리아티`는 검색용 철자 변형으로 보존 | 없음 — 대령은 직함이지 별칭이 아님 | 섹터 D 대령, 킴라박 직속 상관, 아르고 침투부대 지휘관 | 6부 2화 화자명, 킴라박의 직속상관 진술, 기존 프라토·존 웡 기록, 사용자의 섹터 D 확정 | 사용자 확정 `NOVUS_ORDO / MANUS / SECTOR_D` | NHI 문서 비밀 회수 지시, 유회 지원 요청 차단, 부하 제거 시도, Montag-98에게 사망 | `H` 확정 — 같은 대령·현장 통솔 역할인 페초린과 같고 섹터 전체 감독관이라는 근거는 없다는 제안을 사용자가 `2026-08-26` 승인 | 사용자 제공 단독 초상을 배경 제거해 `/assets/npcs/Moriarty-profile.webp`로 준비 | 적용 전 비공개 검토, 현재 전체 공개 완료 | 성별·연령·신장·체중·전체 이름은 `미상` | 힐링 팩터, 거친 동료애, 목적 우선 배신, `KIMLEE`·`YUHOE`·`JOHN_WONG` 관계. `사망 확인`과 근거 사건을 함께 기록하고, `2026-08-09`는 실제 사망일이 아니라 사망 사실을 보존 기록에서 확인한 날. 별도 승인 뒤 신원조회 저장·전체 공개 완료 | ready-for-apply |
| `GWANGCHE_GIRL` | 광체 소녀 — 사용자가 지정한 신원조회 대표 표기. 세션의 `광체`가 고유명인지 존재 분류인지는 미확정으로 보존 | 없음 — `작은 오틸리아` 등은 외형 비교 표현이지 정식 별칭이 아님 | 이르마 곁에서 시유 회유에 참여하고 자신을 `광체`라고 소개한 독립 인격체 / 연구동 이송 예정 대상 | p50~60 시유 회유·주사기 반환, p66~71 이르마 보호, p126 자기소개, p124~137 연구동 이송 결정, 사용자 지정 대표 표기·초상 | 정규 소속 없음. 이르마 측 현장 동행은 관계로만 기록하고 근거 없이 `AHNENERBE` 소속을 등록하지 않음 | 마리아가 노부스 오르도로 함께 가자고 제안해 소녀가 수락했고, 사무총장이 연구동으로 데려가기로 결정. 실제 도착은 확인되지 않음 | 노부스 오르도 요원이 아니므로 내부 권한등급 없음 | 사용자 제공 단독 초상을 배경 제거해 `/assets/npcs/Gwangche-Girl-profile.webp`로 준비 | 적용 전 비공개 검토, 현재 전체 공개 완료 | 여성형 소녀로 관측. 정확한 종족·연령·신장·체중은 `미상` | 마리아 보호, 이르마 충성, 시유 설득 관계는 확인. 오틸리아의 유전자 관련 말은 증언으로만 기록하고 확정하지 않음. 내부 식별자는 링크 연결용이고 화면에는 `광체 소녀`로 표시 | ready-for-apply |

## Personality Evidence Ledger (적용 전 근거 보존본)
...
PDF에는 가로형 장면 삽화가 25개 있다. 그중 반복 구도와 더 설명력이 높은 장면으로 대체할 컷 7개를 제외한 18개를 최종 사용한다. 이 18개가 이전 답변에서 설명 없이 쓴 `TIER-3 장면 프레임`이다. 각 장면은 PDF 안의 `1035×503` 전체 그림을 추가로 자르지 않고 WebP로 변환했으며, 작전보고서와 같은 보고서의 위키판에 동일한 경로·순서·대체 설명·캡션으로 배치했다. 커밋 `6f07257c`로 운영 배포했고, 작전보고서·위키판에서 로고 포함 19개 이미지의 실제 로딩과 캡션을 GM·일반 계정·게스트 화면에서 확인했다. 이번 사용자 제공 초상 2개와 요청에 따라 만든 카탈로그 그림 4개는 이 PDF 장면 삽화와 별개이며, 각각 신원조회와 카탈로그 화면에만 사용한다.

아래 표도 자동 검사 전용이다. `report`는 작전보고서, `report wiki mirror`는 같은 내용을 담는 위키판, `dedicated wiki`는 인물·개체 전용 위키, `catalog`는 아이템 정보, `Dossier/personnel`은 신원조회를 뜻한다. `included`는 사용, `excluded`는 용도 불일치로 제외, `candidate-only`는 보고서에는 사용할 수 있지만 인물·개체 전용 위키의 대표 이미지로는 자동 채택하지 않은 후보라는 뜻이다.
...
- `nosb-s1e6-turning-point-part2-private-staging.json`의 광체 소녀·모리어티 신원조회, 잔광자 위키, 작전보고서, 보고서 위키판을 한 번의 운영 DB 작업으로 비공개 저장했다. 다섯 기록 중 하나라도 실패하면 전부 취소되도록 처리했다.
- 저장 직후 같은 다섯 기록을 DB에서 다시 불러와 이름·공개 범위·사망 상태·관계·이미지·보고서 연결을 저장 예정값과 비교했고 모두 일치했다. 이 문서에서 `저장 뒤 재조회`는 바로 이 확인을 뜻한다.
- GM 화면에서 비공개 보고서와 두 신원조회·두 위키의 이미지와 연결을 확인했다. 보고서에는 로고 1개와 서로 다른 장면 18개가 모두 표시됐다.
- `nosb-s1e6-turning-point-part2-publication.json`의 60개 변경을 한 번의 운영 DB 작업으로 적용했다. 신규 기록 공개, 기존 위키 7건 연결, 기존 인물 21명의 등장 기록, 관계 설명 21개, 성격 관찰 8개, 이르마 코흐의 사망 상태를 함께 반영했다.
- 카탈로그 4건은 전체 사용자에게 보이되 가격 `0`, 판매 불가 상태를 유지했다. 피펫의 광채 표본 1개와 공용 인벤토리의 광채 표본 4개·잔광자 머리 1개·사용된 주사기 1개는 바꾸지 않았고, NHI 문서는 계속 카탈로그에만 있다.
- 로그인한 일반 사용자는 작전보고서와 공개 신원조회·위키·카탈로그를 볼 수 있다. 비로그인 사용자는 기존 서비스 정책에 따라 원문 작전보고서를 볼 수 없지만, 같은 내용을 담은 공개 위키판과 신원조회·잔광자 위키·카탈로그는 볼 수 있다.
- 작전 크레딧·주식·상점 재고·알림·메시지·웹훅은 변경하지 않았다.

## 실제 운영 ERP 반영 현황

1. 카탈로그 4건과 승인된 인벤토리 수량을 먼저 저장하고, 저장 직후 다시 불러와 수량과 보관처를 확인했다.
2. 코드와 이미지 커밋 `6f07257c`를 원격 저장소에 올리고 운영 사이트에 배포했다. 보고서 장면 18개, 로고 1개, 신원조회 초상 2개, 카탈로그 이미지 4개가 모두 정상 응답하는 것을 확인했다.
3. 신규 기록 5건을 비공개로 한 번에 저장하고, 같은 값을 즉시 다시 불러와 비교했다.
4. GM 화면에서 비공개 보고서·신원조회·위키의 이미지, 연결, 공개 표시를 확인했다.
5. 전체 공개 변경 60개를 한 번에 적용했다.
6. 공개 직후 DB에서 다시 불러와 신규 신원조회 2건, 위키 2건, 보고서 1건, 카탈로그 4건, 기존 위키 연결 7건, 기존 인물 등장 기록 21건, 관계 21개, 성격 관찰 8개, 두 인물의 사망 상태가 계획값과 정확히 일치하는지 확인했다.
7. 로그인한 일반 사용자와 게스트 화면에서 공개 범위·이미지·연결·수정 버튼 부재를 확인했다. 게스트에게 원문 보고서가 보이지 않고 공개 위키판이 보이는 것도 현재 서비스의 의도된 권한 규칙과 일치했다.
8. 운영 배포의 최근 오류 기록을 조회했고 오류가 없었다.

`2026-08-26` 현재 승인된 변곡점 2부 운영 ERP 동기화, 전체 공개, 원격 배포와 권한별 화면 확인은 모두 완료됐다.

## 사용자에게 정말 필요한 확인

이번 동기화 범위를 끝내기 위해 사용자가 추가로 확인하거나 제공할 자료는 없다.

모리어티의 정확한 과거 사망일은 아직 알 수 없어, 신원조회에는 실제 사망일이 아니라 공식 기록에서 사망 사실을 확인한 `2026-08-09`를 명시했다. 광체 소녀의 정확한 종족·연령·정식 소속과 실제 연구동 도착 여부, NHI 문서의 현재 소지자도 확인되지 않았으므로 추측해서 채우지 않았다. 이는 남은 운영 작업이 아니라 이후 세션에서 새 근거가 나올 때 보완할 세계관 정보다.
