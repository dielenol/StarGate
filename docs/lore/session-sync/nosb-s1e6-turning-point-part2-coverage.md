...
- 공개 흐름: 원본 표지에는 `CLASSIFICATION · TIER-3 · EYES ONLY`가 적혀 있다. 현재는 작전 보고서, 같은 내용을 담은 위키판, 잔광자 위키, 광채 표본 아이템 정보, 모리어티 신원조회를 모두 비공개 초안으로 준비한다. 내용·연결·이미지가 모두 정리되면 한 번에 전체 공개한다. 실제 운영 ERP에는 아직 저장하지 않았다.

## 이전 답변에서 쓴 말의 정확한 뜻

- `비공개 레퍼런스 2건`: 보고서가 연결할 **잔광자 위키 문서 1건**과 **잔광자 광채 표본 아이템 정보 1건**을 뜻했다. `레퍼런스`라는 말 대신 앞으로 `먼저 만들어 둘 관련 문서`라고 쓴다. 모리어티 비공개 신원조회 초안이 추가되어 현재 먼저 준비할 기록은 총 3건이다.
...
- 공개 기준: 다듬는 동안 전부 비공개로 유지하고, 사용자가 완성됐다고 확인하면 전체 공개한다. 현재 실제 운영 ERP 반영은 승인·실행되지 않았다.
...
|---|---|---|---|---|
| 변곡점 2부 전체 기록 | 141쪽 전 구간과 본문 시작·종료 표기 | 작전보고서, 같은 내용의 위키판 | 보고 순번 `06.5`로 본문·관련 링크 작성. 사용자가 정한 비공개 다듬기 단계에서는 GM만 볼 수 있게 준비 | 비공개 초안 준비됨 |
...
| 모리어티 대령 | 섹터 D 대령, 킴라박의 직속 상관, 문서 탈취·부하 살해 시도, Montag-98에게 사망 | 신규 모리어티 신원조회, 작전보고서, 기존 인물 관계 | `NOVUS_ORDO / MANUS / SECTOR_D`, 권한등급 `H`로 비공개 신원조회 초안을 작성. `사망 확인`으로 표시하고, 날짜 칸에는 실제 사망일이 아니라 이 사실을 보존 기록에서 확인한 `2026-08-09`를 넣음. 초상은 비워 둠 | 실행할 수 없는 검토 파일에 준비됨; D/H·초상 확인 필요 |
...
| 잔광자 광채 표본 | 피펫이 1개를 직접 채취하고 전방 수호대에게 4개를 받음 | 신규 비판매 아이템 정보, 작전보고서 | 아이템 종류는 비공개로 작성. 5개는 개인 소유가 아니라 연구 표본으로 보고 보관처 결정 전 실제 보유 수량을 지급하지 않음 | 비공개 아이템 정보 준비됨 |
| 미확인 광체 소녀 | 이르마 곁에서 시유를 회유하고 주사기를 돌려줌, 자신을 `광체`라고 소개, 마리아의 동행 제안을 수락하고 사무총장이 연구동으로 데려가기로 결정 | 작전보고서, 잔광자 위키; 별도 신원조회 검토 | 독립적으로 말하고 선택하는 인물이므로 신원조회가 가장 적합하다는 추천안. 실제 연구동 도착은 확인되지 않았고, `광체`가 이름인지 종족인지 확정되기 전에는 기술용 영문 식별자를 만들지 않음 | 정체 분류 검토 중 |
...
| NHI 우주 엔진 문서 | 킴라박이 1건을 회수해 모리어티에게 전달했고, 모리어티 사망 뒤 행방 불명 | 작전보고서 | 최종 보관자가 없어 인벤토리에 넣지 않음. 필요하면 나중에 `소재 불명 문헌` 정보로 분리 가능 | 보고서에만 기록 |
| 이르마가 사용한 주사기 | 이르마가 자가 투여한 뒤, 마리아가 이르마가 사용한 주사기 1개를 챙김 | 작전보고서; 증거물 아이템 정보 검토 | 마리아는 마지막 현장 보유자일 뿐 개인 소유자는 아님. 기관 인계·보관처가 확인될 때까지 개인 인벤토리에 넣지 않음 | 비공개 증거물 정보 검토안 |
...
| 모리어티 대령 | `MORIARTY` | 섹터 D 침투부대 지휘, NHI 문서 탈취, 킴라박 배신, Montag-98에게 사망 | 비공개 초안 준비됨; D/H 추천값 확인 필요 |
| 자신을 광체라고 소개한 미확인 소녀 | 아직 식별자 없음 | 시유 회유, 이르마 주사기 반환, 마리아 보호, 연구동 이송 결정. 실제 도착은 확인되지 않음 | 독립 신원조회 추천안; 이름·종족 구분 확인 필요 |
...
| `MARIA` | 미확인 광체 소녀 | 소녀를 보호하고 자신을 어머니로 호명하며 노부스 오르도로 함께 가자고 제안했고, 소녀가 고개를 끄덕여 수락 | 확정 | 소녀 신원조회가 확정되면 양쪽 관계 | 검토안으로만 남김 |

## 회수품·작전 크레딧 처리 판단

`회수했다`는 그 순간 손에 들었다는 뜻이지 개인 소유권이 생겼다는 뜻은 아니다. 아래 표는 누가 무엇을 얼마나 다뤘고 마지막으로 어디까지 확인되는지, ERP에서는 어떻게 처리하는 것이 맞는지를 분리한 것이다.

| 대상 | 수량 | 누가 어떻게 확보했나 | 마지막으로 확인된 상태 | 개인 소유인가 | ERP 처리 추천 | 아직 필요한 정보 |
|---|---:|---|---|---|---|---|
| 잔광자 광채 표본 | 5개 | 피펫이 잔광자에게서 1개를 직접 채취했고, 전방 수호대가 4개를 추가로 피펫에게 전달 | 세션 후반 피펫이 5개를 휴대. 스타크에게는 안전하게 가공한 뒤 나누겠다고만 했고 실제 전달은 없음 | 아님. 현장 운반자가 피펫일 뿐 기관 연구 표본으로 보는 것이 타당 | `잔광자 광채 표본`이라는 비판매 아이템 종류를 비공개로 등록. 실제 수량은 기관 연구 보관 5개가 가장 자연스러운 추천 | 어느 연구실·격리 보관소에 둘지, 현재 ERP에 이미 수량이 기록됐는지 |
| 잔광자 머리 | 1개 | 키아나가 참수했고, 해쉬의 지시로 피펫이 회수했으며 우디가 운반을 도움 | 피펫·우디의 회수 뒤 사무총장이 시체를 백신 연구에 쓰겠다고 설명 | 아님. 피펫이나 우디의 개인 전리품이 아니라 기관 연구 표본 | 비공개·비판매 연구 표본 정보와 기관 보관 1개 추천 | 정확한 연구동·격리실과 머리/시체를 별도 수량으로 셀지 |
| NHI 로켓 추진·우주 엔진 적응 문서 | 1건 | 킴라박이 아르고 기지에서 획득해 모리어티에게 전달 | 문서를 받은 모리어티가 곧 사망했고 이후 문서 회수 기록 없음 | 누구의 소유라고도 확정할 수 없음 | 인벤토리에 넣지 않고 작전보고서에 `행방 불명 문서`로 기록. 필요하면 나중에 비공개 문헌 정보로 분리 | 후속 회수자·현재 보관처 |
| 이르마가 사용한 주사기 | 1개 | 이르마가 자신에게 사용한 뒤, 마리아가 사용한 주사기를 옷 안에 챙김 | 마리아가 마지막 현장 보유자 | 아님. 증거 보존을 위한 회수로 보는 것이 자연스럽고 개인 소유 근거는 없음 | 비공개·비판매 증거물 정보 추천. 기관 인계가 확인될 때까지 마리아 개인 인벤토리에는 넣지 않음 | 마리아가 어느 연구·격리 담당자에게 넘겼는지, 잔류 성분과 보관 조건 |
...
| `MORIARTY` | 모리어티 — 기존 캐논·사용자 표기를 대표 이름으로 유지하고, 이번 PDF의 `모리아티`는 검색용 철자 변형으로 보존 | 없음 — 대령은 직함이지 별칭이 아님 | 섹터 D 대령, 킴라박 직속 상관, 아르고 침투부대 지휘관 | 6부 2화 화자명, 킴라박의 직속상관 진술, 기존 프라토·존 웡 기록 | `NOVUS_ORDO / MANUS / SECTOR_D` 추천안 | NHI 문서 비밀 회수 지시, 유회 지원 요청 차단, 부하 제거 시도, Montag-98에게 사망 | `H` 추천 — 같은 대령·현장 통솔 역할인 페초린과 같고, 섹터 전체 감독관이라는 근거는 없음 | 비워 둠. p011은 2인 작전 장면이라 단독 초상으로 재사용하지 않음 | 다듬는 동안 비공개, 최종 묶음 전체 공개 때 공개 | 성별·연령·신장·체중·전체 이름은 `미상` | 힐링 팩터, 거친 동료애, 목적 우선 배신, `KIMLEE`·`YUHOE`·`JOHN_WONG` 관계. `사망 확인`과 근거 사건을 함께 기록하고, `2026-08-09`는 실제 사망일이 아니라 사망 사실을 보존 기록에서 확인한 날 | blocked |
| 아직 만들지 않음 | 미확인 광체 소녀 — `광체`가 이름인지 종족명인지 미확정 | 없음 — `작은 오틸리아` 등은 외형 비교 표현이지 정식 별칭이 아님 | 이르마 곁에서 시유 회유에 참여하고 자신을 `광체`라고 소개한 독립 인격체 | p50~60 시유 회유·주사기 반환, p66~71 이르마 보호, p126 자기소개, p124~137 연구동 이송 결정 | 과거 활동은 `HOSTILE / AHNENERBE` 쪽이 가장 가까운 추천. 연구동 이송 결정은 새 소속이 아니라 격리·연구 배치 제안 | 마리아가 노부스 오르도로 함께 가자고 제안해 소녀가 수락했고, 사무총장이 연구동으로 데려가기로 결정. 실제 도착은 확인되지 않음 | 외부 연구 대상이라 내부 권한등급 없음 추천 | 독립 단독 초상 없음 | 만들 경우 다듬는 동안 비공개, 최종 묶음 전체 공개 때 공개 | 아동형 소녀로 묘사되지만 정확한 종족·연령·신상은 `미상` | 마리아 보호 관계는 확정. 오틸리아의 유전자 관련 말은 추정 발언이며 확정 사실이 아님 | blocked |
...
PDF에는 가로형 장면 삽화가 25개 있다. 그중 반복 장면과 더 설명력이 높은 장면으로 대체할 컷 7개를 제외한 18개가 최종 사용 후보다. 이 18개가 이전 답변에서 설명 없이 쓴 `TIER-3 장면 프레임`이다. 비공개 검토 중에는 공개 파일 경로에 복사하지 않고 로고만 표시한다. 최종 전체 공개 때 후보 18개를 작전보고서와 위키판에 같은 순서로 싣는 것이 기본안이다.
...
| PDF p011 X241 | PDF p011 X241 | 1035×503 | no — full PDF XObject | report-cutscene | excluded: TIER-3 protected source | excluded: TIER-3 protected source | excluded: report scene only | excluded: no item subject | excluded: 2인 장면이고 초상 승인 없음 | 모리어티와 킴라박의 침투 장면 |
...
- 검토 중에는 사용자가 정한 비공개 다듬기 원칙에 따라 작전보고서를 GM만 볼 수 있게 두고, 보고서 위키판·잔광자 위키·광채 표본 아이템 정보·모리어티 신원조회도 일반 이용자에게 공개하지 않는다.
- 각 등록 파일에는 `현재도 비공개인 같은 항목`만 수정한다는 조건을 넣었다. 같은 식별자의 공개 항목이 이미 있거나 상태가 예상과 다르면 새 내용을 덮어쓰지 않고 작업 전체가 중단되도록 했다.
- 모든 문장·관련 링크·신원조회·아이템 정보·이미지가 정리되면 작전보고서의 열람 등급을 일반 공개로 바꾸고, 나머지 비공개 항목도 공개로 전환한다. 그 뒤 기존 공개 위키와 신원조회에 이번 세션의 등장 기록·관계·성격 관찰을 함께 추가한다.
- 검토 중인 작전보고서와 위키판에는 기존 로고 1개만 표시한다. 최종 전체 공개 때 PDF의 서로 다른 장면 삽화 18개를 양쪽 문서에 같은 순서·설명으로 넣는 것을 기본안으로 한다.
- 현재 비공개 보고서에는 아직 확정되지 않은 모리어티 신원조회를 일반 글자로 적는다. 최종 전체 공개 파일을 새로 만들 때 변곡점 1부, 마지막 대대, 광명회, 광원화 바이러스, 프로젝트 데드 핸드, 스페이스 제로, 뒤집어진 양말, 몬탁 프로젝트, 잔광자, 광채 표본, 모리어티 신원조회를 실제 링크로 연결한다.
- 미확인 광체 소녀는 아직 신원조회 식별자를 만들지 않았으므로 현재 본문에서는 일반 글자로만 적는다. 이름·종족 구분이 정해진 뒤에만 신원조회 링크를 만든다.

## 실제 운영 ERP에 반영할 때의 순서

1. `nosb-s1e6-turning-point-part2-reference-targets.json`에는 사용자 확인과 무관하게 세션 근거가 완성된 잔광자 위키와 광채 표본 아이템 정보 2건의 비공개 준비 내용을 넣었다. 아직 운영 ERP에는 실행하지 않았다.
2. `nosb-s1e6-turning-point-part2-moriarty-dossier.json.pending`에는 모리어티 신원조회만 따로 보존했다. 섹터 D/H와 빈 초상을 사용자가 확인하기 전에는 실행할 수 없다.
3. 관련 기록이 실제로 준비되고 사용자가 별도로 반영을 승인한 뒤에만, `nosb-s1e6-turning-point-part2-sync.json`으로 GM만 보는 작전보고서와 비공개 보고서 위키판을 준비한다.
4. 실행 직전에 같은 식별자의 기존 항목이 있는지와 현재 공개 상태를 다시 확인한다. 이 확인은 새 항목을 중복 생성하거나 공개 문서를 비공개 초안으로 덮어쓰는 일을 막는다.
5. 전체 공개 작업의 참고 목록 55건은 `nosb-s1e6-turning-point-part2-publication-plan.json.pending`에 보존했다. 이 파일은 등록 명령의 실행 대상이 아니며, 이름만 `.json`으로 바꿔 실행해서도 안 된다. 장면 18개와 최종 본문을 함께 넣고, 공개 순서와 연결 검사를 다시 거쳐 실행 파일을 새로 만들어야 한다.
6. 저장 직후 작전보고서·위키·아이템 정보·모리어티 신원조회를 다시 불러와 본문, 공개 범위, `사망 확인` 표시와 확인일, 관련 링크가 의도한 값인지 비교한다. 이 절차는 저장했다고 믿는 대신 실제 저장 결과에 누락·중복이 없는지 확인하는 일이다.
7. 로그인한 테스트 계정과 비로그인 상태에서 관련 링크를 직접 눌러 올바른 화면으로 이동하고, 최종 공개 뒤에는 누구나 볼 수 있는지 확인한다.

이 문서 작성 시점에는 실제 운영 ERP에 아무것도 저장하거나 수정하지 않았다.

## 사용자에게 정말 필요한 확인

1. **모리어티 소속·권한 추천값**: 원문과 기존 프라토·존 웡 기록은 모두 `섹터 D`다. 역할은 섹터 D 전체 감독관이 아니라 아르고 침투부대 지휘 대령으로 확인돼, 같은 대령·현장 통솔자인 페초린과 같은 `H`를 추천한다. 사용자에게 남은 확인은 `섹터 D / H`가 맞는지, 아니면 숨은 설정상 `섹터 C` 또는 섹터 전체 책임자 `M`이 맞는지다.
2. **모리어티 초상**: PDF p011은 모리어티와 킴라박이 함께 있는 작전 장면이라 신원조회용 단독 초상으로 쓰지 않는다. 전용 초상을 받을 때까지 이미지 칸은 비워 둔다.
3. **모리어티 사망 표시**: 사망 자체는 원문에서 확정됐다. 정확한 과거 사망일은 미상이지만, 신원조회 날짜 칸은 실제 사망일이 아니라 공식 기록에서 사망을 확인한 날을 뜻하므로 `2026-08-09`로 처리하는 안을 준비했다. 사용자에게 별도로 과거 사망일을 만들어 달라고 요구하지 않는다.
4. **미확인 광체 소녀**: 독립적으로 말하고 선택하므로 별도 신원조회가 가장 적합하다는 추천안이다. 과거 활동은 이르마 곁이므로 `HOSTILE / AHNENERBE`, 현재는 노부스 오르도 소속이 아니라 연구동 격리·연구 대상, 내부 권한등급 없음이 기본안이다. 세션에서 확인된 것은 연구동으로 데려가기로 한 결정까지이며 실제 도착은 확인되지 않았다. `광체`가 개인 이름인지 종족명인지와 전용 초상 여부만 확인하면 된다.
5. **연구 표본 보관처**: 광채 표본 5개와 잔광자 머리 1개는 피펫·우디 개인 인벤토리가 아니라 기관 연구 보관으로 추천한다. 정확히 어느 연구동·격리실에 둘지 정해져야 실제 보유 수량을 넣을 수 있다. 사용한 주사기 1개는 마리아 개인 소유가 아니라 기관 증거물 보관을 추천한다.
작전 크레딧 1,000은 사용자 결정 항목에서 제외했다. 세션상 사용은 기록하되 이미 수동 반영됐을 가능성을 배제할 수 없어 추가 차감하지 않는 것이 최종 판단이다.
