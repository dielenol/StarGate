...
- `비공개 초안 저장`: 기록을 일반 이용자에게 공개하지 않은 상태로 ERP에 저장해 검토하는 일이다. 현재 카탈로그 4건만 이 단계까지 완료했고, 작전 보고서·위키·신원조회는 아직 운영 ERP에 저장하지 않았다.
...
- 공개 기준: 다듬는 동안 전부 비공개로 유지하고, 사용자가 완성됐다고 확인하면 전체 공개한다. 카탈로그 4건과 승인된 인벤토리 수량은 실행 직전 값을 제시해 승인받은 뒤 `2026-08-26`에 반영했다. 작전 보고서·위키·신원조회는 아직 운영 ERP에 반영하지 않았다.
...
| 잔광자 머리 | 키아나가 참수하고 해쉬가 회수를 지시했으며 피펫이 회수하고 우디가 운반을 도움 | 신규 비판매 카탈로그 항목, 공용 인벤토리, 작전보고서 | 사용자 지시에 따라 생체 표본 1개를 공용 인벤토리에 귀속 | `2026-08-26` 운영 ERP 반영 및 재조회 완료: 공용 1개 |
| 광체 소녀 | 이르마 곁에서 시유를 회유하고 주사기를 돌려줌, 자신을 `광체`라고 소개, 마리아의 동행 제안을 수락하고 사무총장이 연구동으로 데려가기로 결정 | 신규 광체 소녀 신원조회, 작전보고서, 잔광자 위키 | 사용자 지정 대표 표기 `광체 소녀`와 제공 초상을 사용한다. `광체`가 고유명인지 존재 분류인지는 미확정으로 남기고, 정규 소속·내부 권한등급은 만들지 않는다. 연구동 이송은 결정만 기록하고 실제 도착은 단정하지 않음 | 비공개 신원조회 문서·등록 자료·초상 준비됨; 운영 ERP 저장은 별도 승인 전까지 보류 |
...
| NHI 우주 엔진 문서 | 킴라박이 1건을 회수해 모리어티에게 전달했고, 모리어티 사망 뒤 행방 불명 | 신규 비판매 카탈로그 항목, 작전보고서 | 사용자 지시에 따라 문서의 존재·내용·마지막 확인 경로만 카탈로그에 기록. 현재 보관자가 없으므로 개인·공용 인벤토리 수량은 모두 0으로 유지 | `2026-08-26` 카탈로그 반영 및 재조회 완료; 개인·공용 인벤토리 0 유지 |
| 이르마가 사용한 주사기 | 이르마가 자가 투여한 뒤, 마리아가 사용된 주사기 1개를 챙김 | 신규 비판매 카탈로그 항목, 공용 인벤토리, 작전보고서 | 마리아를 마지막 현장 보유자로 기록하고, 사용자 지시에 따라 사용된 현장 물증 1개를 공용 인벤토리에 귀속. 소비품이나 재사용 가능한 주사기로 취급하지 않음 | `2026-08-26` 운영 ERP 반영 및 재조회 완료: 공용 1개 |
| 엘 볼라도르 폭탄 칩셋 | 목에 강제 주입, 명령 위반 시 즉결처형 언급 | 작전보고서, ACCEL·Mr. 오드 관계 | 생체 삽입 상태를 서술하고 별도 장비나 보유품으로 만들지 않음 | 처리 판단 끝남 |
| 작전 크레딧 1,000 | 마리아가 전투원 2명 호출에 1,000 사용을 선언했고 실제 합류 확인 | 작전보고서 | 세션 당시 자동 차감 흔적은 없지만 GM이 잔액을 직접 조정했는지는 남은 기록만으로 확정할 수 없다. 중복 차감 위험 때문에 새로 차감하지 않고, 세션에서 사용했다는 사실만 보고서에 기록 | 보고서 초안에만 기록; 운영 ERP 미저장, 추가 차감 안 함 |
...
| 모리어티 대령 | `MORIARTY` | 섹터 D 침투부대 지휘, NHI 문서 탈취, 킴라박 배신, Montag-98에게 사망 | 섹터 D·전용 초상·권한등급 `H` 확정. 비공개 신원조회 등록 자료는 준비됐으며 운영 ERP 저장은 별도 승인 전까지 보류 |
...
|---|---:|---|---|---|---|---|
| 잔광자 광채 표본 | 5개 | 피펫이 잔광자에게서 1개를 직접 채취했고, 전방 수호대가 4개를 추가로 피펫에게 전달 | 세션 후반 피펫이 5개를 휴대. 스타크에게는 안전하게 가공한 뒤 나누겠다고만 했고 실제 전달은 없음 | 직접 채취한 1개만 사용자 지시로 피펫 보유 처리. 나머지 4개는 공용 연구 자산 | 비공개·비판매 카탈로그 1건을 만들고 피펫 개인 인벤토리 `0→1`, 공용 인벤토리 `0→4`로 반영 | `2026-08-26` DB 재조회와 ERP 화면에서 피펫 1개·공용 4개 확인 완료 |
| 잔광자 머리 | 1개 | 키아나가 참수했고, 해쉬의 지시로 피펫이 회수했으며 우디가 운반을 도움 | 피펫·우디의 회수 뒤 사무총장이 시체를 백신 연구에 쓰겠다고 설명 | 아님. 피펫이나 우디의 개인 전리품이 아니라 공용 연구 표본 | 비공개·비판매 카탈로그를 만들고 공용 인벤토리 `0→1`로 반영 | `2026-08-26` DB 재조회와 ERP 화면에서 공용 1개 확인 완료 |
| NHI 로켓 추진·우주 엔진 적응 문서 | 1건 | 킴라박이 아르고 기지에서 획득해 모리어티에게 전달 | 문서를 받은 모리어티가 곧 사망했고 이후 문서 회수 기록 없음 | 누구의 소유라고도 확정할 수 없음 | 비공개·비판매 카탈로그에 문서의 존재와 마지막 확인 경로만 등록. 개인·공용 인벤토리는 모두 `0` 유지 | `2026-08-26` 카탈로그 등록과 인벤토리 0 유지 확인 완료. 후속 회수자·현재 보관처는 세계관상 미확정 |
| 이르마가 사용한 주사기 | 1개 | 이르마가 자신에게 사용한 뒤, 마리아가 사용된 주사기를 옷 안에 챙김 | 마리아가 마지막 현장 보유자 | 아님. 개인 소유물이 아니라 사용 후 현장 물증 | 사용 흔적이 보이는 비공개·비판매 카탈로그를 만들고 공용 인벤토리 `0→1`로 반영 | `2026-08-26` DB 재조회와 ERP 화면에서 공용 1개 확인 완료. 잔류 성분과 보관 조건은 후속 연구 정보로 남김 |
