...
`WORKER_OUTBOX_KINDS`는 별도의 opt-in 목록이다. 지원 값은 `GM_ADMIN_AUDIT`, `CHARACTER_EDIT_WEBHOOK`, `EQUIPMENT_WORKSHOP_WEBHOOK`, `SHOP_REORDER_REQUEST_WEBHOOK`, `SHOP_REORDER_FULFILLED_WEBHOOK`, `STOCK_MANUAL_INTERVENTION_WEBHOOK`, `PLAYER_TRADE_DM`이다. 활성화한 kind에 필요한 webhook/bot token이 없으면 worker는 outbox를 claim하기 전에 기동을 거부한다.
