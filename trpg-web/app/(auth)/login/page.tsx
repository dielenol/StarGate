...
const ERROR_MESSAGES: Record<string, string> = {
  AccessDenied: "이 봇이 운영하는 길드의 멤버만 로그인할 수 있습니다.",
  Configuration: "인증 설정 오류가 발생했습니다. 운영자에게 문의하세요.",
    ...
    <main className={styles.login}>
      <div className={styles.login__panel}>
        ...
        <form
          action={async () => {
            "use server";
            await signIn("discord", { redirectTo: "/calendar" });
          }}
        >
          <button className={styles.login__button} type="submit">
            <svg
              className={styles["login__button-icon"]}
              viewBox="0 0 24 24"
              aria-hidden="true"
            >
