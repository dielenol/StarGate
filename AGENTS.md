...
`.Codex/rules/commit-convention.md` 참조. 핵심:
- `<type>(<scope>): <한국어 제목>` + 불릿 본문
- Co-Authored-By 자동 서명 **금지**

### 자동 커밋 운영
- 파일을 변경하는 작업에서는 커밋을 별도 후속 요청이 아니라 작업 완료 조건의 일부로 취급한다.
- 하나의 작업 단위가 완료되면, 사용자가 명시적으로 커밋 금지를 요청하지 않은 한 `auto-commit` 스킬을 사용해 변경 사항을 커밋한다.
- 기본 전략은 스플릿 커밋이다. 기능, 문서, 데이터, 스타일, 리팩터링처럼 성격이 다른 변경은 작업 단위별로 나누어 커밋한다.
- 단일 커밋은 모든 변경이 같은 목적과 같은 scope에 속하고 분리했을 때 의미가 흐려지는 경우에만 사용한다.
- 커밋 전에는 해당 작업 단위에 맞는 가장 좁은 검증을 실행하고 결과를 확인한다. 검증을 실행할 수 없거나 실패하면 그 상태를 보고하고 무리하게 숨기지 않는다.
- 사용자 또는 다른 작업으로 보이는 기존 변경은 섞지 않는다. 커밋 대상은 방금 완료한 작업 단위의 파일로 한정한다.
- 스테이징은 `git add -A`를 쓰지 않고 커밋 대상 파일을 명시한다. `.env`, credentials, 시크릿 파일, 임시 산출물은 커밋하지 않는다.
- Git 추적 파일 변경이 없는 DB/외부 상태 작업은 커밋 대상이 없음을 명시적으로 보고한다.
- 파일 변경 작업의 최종 응답에는 생성한 커밋 해시 또는 커밋하지 않은 구체적 사유를 포함한다.
