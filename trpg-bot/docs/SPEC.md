...
- Discord에서 원본을 확대해 읽을 수 있도록 캘린더 레이아웃 자체를 1920px 가로형으로 렌더링한다.
- 현재 월을 조회할 때 오늘 날짜를 강조한다.
- 날짜별 세션은 시작 시각 순으로 최대 3건을 표시하고 초과분은 `+N more`로 표시한다.
- 제목은 축약하지 않고 캘린더 칸 안에서 여러 줄로 표시한다.
...
4. 음량이 기본값(100%)이면 플레이어 인라인 볼륨과 오디오 필터를 사용하지 않는다. `/음악 볼륨`으로 100%가 아닌 값을 지정하면 해당 세션은 FFmpeg `volume` 필터를 거치는 변환 경로를 사용한다. Opus 인코더 의존성을 추가하지 않고 음량을 조절할 수 있는 경로가 FFmpeg 뿐이기 때문이다.
4-1. 재생 중 음량을 바꾸면 현재 곡을 재생 위치(`-ss`)부터 다시 연다. 이미 흐르는 스트림에는 필터를 끼울 수 없기 때문이다. 라이브 스트림은 위치 탐색이 불가해 처음부터 다시 연다. 음량 설정은 세션 메모리에만 유지된다.
5. Discord 음성 연결은 `@discordjs/voice`의 기본 DAVE E2EE를 사용한다.
6. 미디어 URL 해석은 기본 player client `visionos`를 사용하고 `YT_DLP_PLAYER_CLIENTS`로 재정의할 수 있다. Docker 이미지는 bgutil POT provider를 같은 컨테이너에서 함께 띄우므로 PO token을 요구하는 client도 사용할 수 있다.
7. googlevideo가 미디어 URL을 HTTP 403으로 거부하면 같은 client로 재해석해도 복구되지 않으므로, 다른 player client(`web_safari`, PO token 사용)와 완화된 포맷 조건(HLS·m4a 허용)을 쓰는 폴백 프로필로 한 번 교체해 다시 해석한다. 403이 아닌 실패는 서명 URL 만료로 보고 같은 프로필로 한 번만 재해석하며, 준비 시도는 총 3회로 제한한다.
...
- 음악 셔플·EQ·영구 대기열은 지원하지 않는다. 볼륨은 `/음악 볼륨`으로 세션 단위(메모리)로만 조절하며 재시작 시 100%로 돌아간다.
