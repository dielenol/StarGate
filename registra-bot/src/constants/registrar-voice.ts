...
export const Help = {
  ...
  playerFields: [
    ...
    { name: "​", value: "━━ **크레딧** ━━" },
    {
      name: "◆ `/잔액`",
      value:
        "본인 **메인 캐릭터**의 크레딧 잔액과 **최근 5건** 거래 내역을 **본인에게만 보이는(비공개)** 형태로 조회합니다. 등재된 메인 캐릭터가 없으면 운영자에게 문의 부탁드립니다.",
    },
  ],
  ...
  adminFields: [
    ...
    // ── 크레딧 운영 (GM) ──
    { name: "​", value: "━━ **크레딧 운영 (GM)** ━━" },
    {
      name: "◆ `/크레딧 지급`",
      value:
        "지정 멤버의 **메인 캐릭터**에 크레딧을 지급합니다. 옵션: `대상`(필수)·`금액`(필수)·`사유`(선택). 결과는 잔액 변동(이전 → 이후)과 함께 보고됩니다.",
      inline: true,
    },
    {
      name: "◆ `/크레딧 차감`",
      value:
        "지정 멤버의 **메인 캐릭터**에서 크레딧을 차감합니다. 운영 재량으로 **음수 잔액 허용**. 옵션: `대상`(필수)·`금액`(필수)·`사유`(선택).",
      inline: true,
    },
    {
      name: "◆ `/크레딧 전체지급`",
      value:
        "ledger 보유 **운영 캐릭터 전원**에게 동일 금액을 일괄 지급합니다. 옵션: `금액`(필수)·`사유`(선택). 부분 실패는 별도 목록으로 보고됩니다.",
      inline: true,
    },
    {
      name: "◆ `/크레딧 작전지급`",
      value:
        "**작전 크레딧 풀**에 입금합니다(운영 자금원). 옵션: `금액`(필수)·`사유`(선택). 풀 잔액 변동이 보고됩니다.",
      inline: true,
    },
    {
      name: "◆ `/크레딧 작전차감`",
      value:
        "**작전 크레딧 풀**에서 출금합니다. 풀 잔액 부족 시 거부됩니다. 옵션: `금액`(필수)·`사유`(선택).",
      inline: true,
    },
    {
      name: "◆ `/크레딧 조회`",
